# Section 1: Multiline TUI Input

Replace single-line `ink-text-input` with a custom multiline TextInput.

## Background

Current `tui/src/components/chat/input.tsx` uses `ink-text-input` — single-line only. Enter submits (or Ctrl+Enter after our change). User wants Enter=newline, Ctrl+Enter=submit, plus cursor movement, paste, and history.

Hermes' `textInput.tsx` (at `inspiration/hermes-agent/ui-tui/src/components/textInput.tsx`) is a full 1088-line custom implementation. We're building a focused subset.

## Implementation

### Step 1: Create multiline TextInput
...
Replace `ink-text-input` with the new `MultilineInput`:
...
### Step 3: Remove ink-text-input dependency

After verifying the new component works, remove from `package.json`:
```bash
cd /home/harsh/docker/overkill/tui && npm uninstall ink-text-input
```
...
Test manually:
- Type multiple lines (Enter = newline)
