...
function useVersion(backend: ReturnType<typeof useBackend>["backend"]): string {
  const [version, setVersion] = useState("v3");

  useEffect(() => {
    backend
      .call<{ version: number }>("health.check")
      .then((result) => setVersion("v" + result.version))
      .catch(() => {}); // use default "v3" if health check fails
  }, [backend]);
...
export function App(): React.JSX.Element {
  ...
  const version = useVersion(backend);
  ...
  // Memoize commandCtx so useMemo below is stable across renders when
  // the individual values haven't changed.
  ...
  useKeybindings(
    {
      ...
      "global:cycleThinking": () => {
        ...
        backend
          .call("thinking.set_level", { level: next })
          .catch((err: unknown) => {
            log.error("thinking.set_level failed:", err);
          });
      ...
      "global:toggleMode": () => {
        ...
        setAgentMode(next);
        backend.call("mode.set", { mode: next }).catch((err: unknown) => {
          log.error("mode.set failed:", err);
        });
  ...
  if (configLoading) {
    return (
      ...
      >
        <Text color={theme.warning}>Checking configuration...</Text>
      </Box>
    );
  }
  ...
  return (
    <KeybindingProvider>
      <Box flexDirection="column" width="100%" height="100%">
        ...
        <Box flexDirection="row" flexGrow={1} width="100%" overflow="hidden">
          ...
          <Sidebar
            ...
            branch={gitBranch}
            isInputFocused={!isLoading && !isDialogOpen}
            files={fileChanges.map(
              (fc: { path: string; added: number; removed: number }) => ({
            ...
            onTabChange={setTab}
            panels={{
              todos: <TodoPanel active={sidebarVisible} backend={backend} />,
              skills: <SkillsPanel active={sidebarVisible} backend={backend} />,
              sessions: <SessionPanel backend={backend} />,
              agents: <SubagentPanel backend={backend} />,
              "self-eval": <SelfEvalPanel backend={backend} />,
              tests: <TestPanel backend={backend} />,
              wizard: <WizardPanel backend={backend} />,
              queue: <QueuePanel backend={backend} />,
            }}
          />
        </Box>
      ...
      <SteerDialog
        ...
        onClose={close}
        onSubmit={(msg) => {
          backend.steer(sessionId, msg).catch((err: unknown) => {
            log.error("steer failed:", err);
          });
        }}
      />
      <SettingsPanel
      ...
      <ClarifyDialog
        ...
        onCancel={() => {
          ...
          backend
            ...
            .catch((err: unknown) => {
              log.error("agent.answer cancel failed:", err);
            });
        }}
      />
    </KeybindingProvider>
