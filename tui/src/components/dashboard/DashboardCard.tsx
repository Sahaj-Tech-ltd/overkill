...
function goalBadgeVariant(status: GoalData["status"]): "success" | "warning" | "info" | "danger" {
  switch (status) {
    case "active":          return "success";
    case "blocked":         return "warning";
    case "complete":        return "info";
    case "budget_limited":  return "danger";
  }
}
...
function BudgetBar({
  ...
  const fillColor = ratio > 0.9 ? theme.error : ratio > 0.7 ? theme.warning : theme.success;
...
export function DashboardCard({ open, onClose }: DashboardCardProps): React.JSX.Element | null {
  ...
  const load = () => {
    fetchGoal()
      .then((g) => { setGoal(g); setGoalError(null); })
      .catch((e: unknown) => { setGoalError((e as Error).message); });

    fetchPlan()
      .then((p) => { setPlan(p); setPlanError(null); })
      .catch((e: unknown) => { setPlanError((e as Error).message); });
  };
  ...
  return (
    <DialogContainer open={open} onClose={onClose} title="Dashboard">
      ...
      <Box flexDirection="column">
        ...
        {/* ── ACTIVE GOAL ──────────────────────────────────────────── */}
        <Box marginBottom={1}>
          <Text bold color={theme.accent}>Active Goal</Text>
        </Box>
          ...
          <Box flexDirection="column" marginBottom={1}>
            ...
            <Box>
              <Text color={theme.muted}>
                Tokens: {formatTokens(goal.tokens_used)} | Time: {formatTime(goal.time_used_s)}
              </Text>
            </Box>
          </Box>
        ) : (
        ...
        {/* ── PLAN ─────────────────────────────────────────────────── */}
        <Box marginBottom={1}>
          <Text bold color={theme.accent}>Plan</Text>
          {planTotal > 0 ? (
            <Text color={theme.muted}>  ({planDone}/{planTotal} done{planRemaining ? `, ${planTotal - planDone} remaining` : ""})</Text>
          ) : null}
        </Box>
        ...
        {/* ── BUDGET ───────────────────────────────────────────────── */}
        <Box marginBottom={1}>
          <Text bold color={theme.accent}>Budget</Text>
        </Box>
        ...
        {goalError && !goal ? (
          ...
          <BudgetBar used={goal.tokens_used} budget={goal.token_budget} theme={theme} />
        ) : (
          <Box>
            <Text color={theme.muted}>—</Text>
          </Box>
