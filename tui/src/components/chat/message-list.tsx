...
export function MessageList({
  ...
  useInput((input, key) => {
    if (key.pageUp) {
      onScrollChange(
        Math.min(messages.length - 1, scrollOffset + 5),
      );
    }
    if (key.pageDown) {
    ...
    if (key.upArrow && !key.ctrl && !key.meta) {
      onScrollChange(
        Math.min(messages.length - 1, scrollOffset + 1),
      );
    }
  ...
  useScrollHandler((direction) => {
    if (direction === "up") {
      onScrollChange(
        Math.min(messages.length - 1, scrollOffset + 3),
      );
    } else {
        ...
        {visibleMessages.map((msg, i) => (
          <Box key={msg.id ?? `msg-${i}`} marginBottom={1}>
            <MessageBubble
              ...
              content={msg.content}
              terminalWidth={
                showFileBar ? terminalWidth - 26 : terminalWidth
              }
