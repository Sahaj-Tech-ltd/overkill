...
function parseMarkdown(text: string): TextSegment[] {
  ...
  while (remaining.length > 0) {
    // Find all possible markers
    const markers: { pos: number; type: string; markerLen: number }[] = [];

    // Headers (# at start of line or after newline)
    const headerMatch = remaining.match(/(?:^|\n)(#{1,6})\s/);
    if (headerMatch && headerMatch.index !== undefined) {
      const hPos =
        headerMatch.index! + headerMatch[0].length - headerMatch[1].length - 1;
      markers.push({
        pos: hPos,
        type: "heading",
        markerLen: headerMatch[1].length + 1,
      });
    }
    ...
    const italicUnderscore = remaining.indexOf("_");
    const strikethroughStart = remaining.indexOf("~~");
    const inlineCodeStart = remaining.indexOf("`");
    const linkStart = remaining.indexOf("[");
    const explicitLink = remaining.match(/\[([^\]]+)\]\(([^)]+)\)/);

    if (codeBlockStart !== -1)
      markers.push({ pos: codeBlockStart, type: "codeblock", markerLen: 3 });
    if (boldStart !== -1)
      markers.push({ pos: boldStart, type: "bold", markerLen: 2 });
    // Only count * as italic if it's not part of ** (bold handled first)
    if (
      italicStar !== -1 &&
      italicStar !== boldStart &&
      remaining.charAt(italicStar + 1) !== "*"
    ) {
      markers.push({ pos: italicStar, type: "italic", markerLen: 1 });
    }
    if (
      italicUnderscore !== -1 &&
      remaining.charAt(italicUnderscore - 1) !== "_"
    ) {
      markers.push({ pos: italicUnderscore, type: "italic", markerLen: 1 });
    }
    if (strikethroughStart !== -1)
      markers.push({
        pos: strikethroughStart,
        type: "strikethrough",
        markerLen: 2,
      });
    if (inlineCodeStart !== -1 && inlineCodeStart !== codeBlockStart) {
      markers.push({ pos: inlineCodeStart, type: "code", markerLen: 1 });
    }
    if (
      linkStart !== -1 &&
      remaining.charAt(linkStart + 1) !== "[" &&
      explicitLink
    ) {
      markers.push({ pos: linkStart, type: "link", markerLen: 1 });
    }
    ...
    if (first.type === "heading") {
      ...
      const endIdx = newlineIdx === -1 ? remaining.length : newlineIdx;
      // Skip the # markers and space
      const hLevel = remaining.match(/^(#{1,6})\s/)![1].length;
      segments.push({
        type: "heading",
        content: remaining.slice(hLevel + 1, endIdx),
      });
      ...
      segments.push({ type: "italic", content: remaining.slice(1, endIdx) });
      remaining = remaining.slice(endIdx + 1);
    } else if (first.type === "strikethrough") {
      const endIdx = remaining.indexOf("~~", 2);
      if (endIdx === -1) {
        segments.push({ type: "text", content: remaining });
        break;
      ...
      segments.push({
        type: "strikethrough",
        content: remaining.slice(2, endIdx),
      });
      remaining = remaining.slice(endIdx + 2);
    } else {
      // code
...
function renderSegment(
  ...
  switch (segment.type) {
    case "heading":
      return (
        <Text key={key} bold color={theme.highlight}>
          {segment.content}
        </Text>
      );
    case "bold":
    ...
    case "link":
      return (
        <Text key={key} underline color={theme.accent}>
          {segment.content}
        </Text>
      );
    case "code":
        ...
        >
          {highlightedLines.length > 0
            ? highlightedLines.map(
                (
                  line: Array<{ text: string; color: string | undefined }>,
                  i: number,
                ) => (
                  <Text key={i}>
                    {line.length > 0 ? (
                      line.map(
                        (
                          token: { text: string; color: string | undefined },
                          j: number,
                        ) => (
      ...
      {!streaming && turnDuration != null && turnDuration > 0 && (
        <Box marginTop={1}>
          <Text color={theme.muted}>
            ⏱ Turn completed in {formatDuration(turnDuration)}
          </Text>
        </Box>
      )}
      {streaming && (
