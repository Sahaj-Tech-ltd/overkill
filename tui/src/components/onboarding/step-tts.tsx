...
/** Validate TTS API key format. Returns error message or null if valid. */
function validateTTSKey(providerId: string, key: string): string | null {
  ...
  switch (providerId) {
    case "openai-tts":
      ...
      if (trimmed.length < 20)
        return "OpenAI key must be at least 20 characters";
      return null;
    case "elevenlabs":
      if (trimmed.length < 20)
        return "ElevenLabs key must be at least 20 characters";
      return null;
    case "play.ht":
      if (trimmed.length === 0) return "API key cannot be empty";
      return null;
    default:
      // Generic: require non-empty when a key field is shown
      if (trimmed.length === 0) return "API key cannot be empty";
      return null;
  }
}
...
export function StepTTS({
  ...
  const [ttsKeyError, setTtsKeyError] = useState<string | null>(null);
  ...
  const confirmKey = useCallback(() => {
    ...
    // Only validate when the provider actually requires a key
    if (providerRequiresKey(selected.id)) {
      const err = validateTTSKey(selected.id, apiKeyInput);
      if (err) {
        setTtsKeyError(err);
        return;
      }
  ...
  useInput((input, key) => {
    ...
    if (key.upArrow) {
      ...
      setSelectedIdx((prev) => Math.min(TTS_PROVIDERS.length - 1, prev + 1));
    } else if (key.return) {
      handleSelect();
    ...
    <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginBottom={1}>
        {TTS_PROVIDERS.map((provider, i) => {
          ...
          const isSelected =
            provider.id === "none" ? !tts : tts?.provider === provider.id;

          return (
            <Box key={provider.id}>
              <Text color={isHighlighted && !editingKey ? "cyan" : undefined}>
      ...
      {editingKey && (
        <Box marginY={1} flexDirection="column">
          ...
          {ttsKeyError && (
            <Box>
              <Text color={theme.error}>✗ {ttsKeyError}</Text>
            </Box>
          )}
