...
function getVisionModels(providers: OnboardingProviderConfig[]): ModelInfo[] {
...
export function StepVision({
  ...
  const [selectedIdx, setSelectedIdx] = useState(() => {
    ...
    // Only accept if the provider is actually configured
    const modelProvider = visionProvider.split("/")[0];
    const providerExists = providers.some(
      (p) => p.name.toLowerCase() === modelProvider.toLowerCase(),
    );
    if (!providerExists) return 0;
  ...
  }, [visionProvider, visionModels, providers]);
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      if (selectedIdx === 0) {
        ...
        const model = visionModels[selectedIdx - 1];
        // Validate the provider exists in the user's configured providers
        const modelProvider = model.id.split("/")[0];
        const providerExists = providers.some(
          (p) => p.name.toLowerCase() === modelProvider.toLowerCase(),
        );
        if (!providerExists) {
          // This shouldn't happen with normal navigation, but guard against
          // stale/incorrect external visionProvider values
          return;
        }
        setVisionProvider(model.id);
      }
  ...
  return (
    <Box flexDirection="column">
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text dimColor>
          Select a vision-capable model for image understanding. You can use
          your default model if it supports vision.
        </Text>
      </Box>
      ...
      {providers.length === 0 ? (
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {!hasModels && (
            <Box marginY={1}>
              <Text dimColor>
                No known vision-capable models for your providers. Select "Same
                as default model" to use your default model.
              </Text>
            </Box>
          )}
        </Box>
      ...
      {visionProvider && (
        <Box marginBottom={1}>
          <Text color={theme.success}>Vision: {visionProvider}</Text>
        </Box>
      )}
      {visionProvider === "" && (
        <Box marginBottom={1}>
          <Text dimColor>Using default model for vision tasks.</Text>
        </Box>
      )}
