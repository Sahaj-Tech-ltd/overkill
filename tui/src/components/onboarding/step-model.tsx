...
export function StepModel({
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      setSelectedIdx((prev) =>
        Math.min(Math.max(0, models.length - 1), prev + 1),
      );
    } else if (key.return) {
      const safeIdx = Math.min(selectedIdx, models.length - 1);
      if (safeIdx >= 0 && models[safeIdx]) {
        setDefaultModel(models[safeIdx].id);
      }
  ...
  return (
    <Box flexDirection="column">
      ...
      {defaultModel && (
        <Box marginBottom={1}>
          <Text color={theme.success}>Default: {defaultModel}</Text>
        </Box>
      )}
