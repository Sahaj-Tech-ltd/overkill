      ...
      while (buffer.length > 0) {
        ...
        if (sgrStart === -1) {
          ...
          if (buffer[escIdx + 1] === "[") {
            ...
            buffer = buffer.slice(0, escIdx) + buffer.slice(escIdx + csiEnd + 1);
            continue;
          }
        ...
        const effectiveEnd =
          endIdx === -1 ? endIdxM : endIdxM === -1 ? endIdx : Math.min(endIdx, endIdxM);
...
export function useMousePosition(): MouseEvent | null {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (current && (pos === null || current.x !== pos.x || current.y !== pos.y)) {
        setPos(current);
      }
    }, 50);
