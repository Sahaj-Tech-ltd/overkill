...
func (s *Server) pairingStore() *security.PairingStore {
	// TODO: wire pairing store into Server.Config once the config struct
	// gains a PairingStore field. For now, return nil — the schema and
	// handlers are ready; the pairing store is constructed by the CLI
	// and injected into the server config.
	return nil
}
