...
type Config struct {
	...
	PairingStore *security.PairingStore // optional; powers the /api/pairing/* endpoints
	Provider     string
	Version      string
}
...
func (s *Server) Handler() http.Handler {
	...
	// Unified flow system (§7.1.5-7.1.8) — unregistered until task-flow storage is wired.
	// s.flowRoutes(mux)
	s.pairingRoutes(mux)
	s.canvasRoutes(mux)
	return mux
...
func isLoopbackAddr(addr string) bool {
	...
	if err != nil {
		// SplitHostPort failed — addr is malformed. Use a conservative
		// check: only match well-known loopback prefixes followed by ':'.
		// This covers bare addresses like "localhost:8080" but not
		// "localhost.evil.com:8080" (which SplitHostPort would have parsed).
		if strings.HasPrefix(addr, "127.0.0.1:") || strings.HasPrefix(addr, "[::1]:") || addr == "localhost" || strings.HasPrefix(addr, "localhost:") {
			return true
		}
		return false
	}
...
func (s *Server) handleIndex(sub fs.FS) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// First-load token plumbing: ?t=… sets the cookie only once.
		// Subsequent ?t= values from the same session are ignored to
		// prevent cookie-overwrite attacks via crafted links.
		if t := r.URL.Query().Get("t"); t != "" {
			if _, err := r.Cookie("overkill-token"); err == http.ErrNoCookie {
				http.SetCookie(w, &http.Cookie{
					Name: "overkill-token", Value: t, Path: "/",
					MaxAge: 60 * 60 * 24 * 365, SameSite: http.SameSiteStrictMode,
