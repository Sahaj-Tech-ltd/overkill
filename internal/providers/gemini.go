		...
		for idx, part := range result.Candidates[0].Content.Parts {
			...
			if part.FunctionCall != nil {
				...
				if len(args) == 0 {
					...
					// Incomplete JSON from a dropped connection —
					// skip to avoid downstream unmarshal failures.
					log.Warn().Int("idx", idx).Str("name", part.FunctionCall.Name).Msg("providers/gemini: function call args not valid JSON, skipping")
					continue
				}
				id := fmt.Sprintf("gemini-fc-%d-%s", idx, part.FunctionCall.Name)
...
func (p *GeminiProvider) readSSEStream(ctx context.Context, body io.Reader, ch chan<- Chunk) {
	...
	for scanner.Scan() {
		...
		if len(chunk.Candidates) > 0 {
			...
			// Track finish sentinel — Gemini signals completion via finishReason STOP.
	...
	if err := scanner.Err(); err != nil {
		logSendChunk(ctx, ch, Chunk{Err: fmt.Errorf("gemini stream: %w", err)})
		return
	}
	// L18: require a STOP/MAX_TOKENS sentinel before emitting Done. Clean EOF
	// without a finish reason is an unexpected truncation, not a completion.
	if !sawStop {
		logSendChunk(ctx, ch, Chunk{Err: fmt.Errorf("gemini stream: unexpected EOF without finish reason")})
		return
	}
