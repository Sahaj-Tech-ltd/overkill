...
type FactoryConfig struct {
	...
	AuthType string // "oauth" to use stored OAuth tokens via ResolveOAuthAPIKey
}

func NewProvider(cfg FactoryConfig) (Provider, error) {
	// OAuth: resolve stored token when AuthType == "oauth" and no explicit API key.
	if cfg.AuthType == "oauth" && cfg.APIKey == "" {
		cfg.APIKey = ResolveOAuthAPIKey(cfg.Name)
	}
...
func newProviderRaw(cfg FactoryConfig) (Provider, error) {
	// Providers that require authentication must have a non-empty API key.
	// Self-hosted / local providers (ollama, lmstudio, local) are exempt.
	if providerRequiresAuth(cfg.Type) && cfg.APIKey == "" {
		return nil, fmt.Errorf("providers: %s requires an API key — none provided (set api_key in config or %s env var)",
			cfg.Type, CanonicalAPIKeyEnv(cfg.Type))
	}
...
// providerRequiresAuth returns true for provider types that need an API key.
// Self-hosted / local providers (ollama, lmstudio, local) are exempt.
func providerRequiresAuth(providerType string) bool {
	switch providerType {
