...
// AgentPromptCompressor wraps a PromptCompressor to satisfy agent.PromptCompressor.
// The agent expects Compress(ctx, prompt) (string, int, error) where int = tokens saved.
type AgentPromptCompressor struct {
	inner *PromptCompressor
}

// NewAgentPromptCompressor builds the adapter. model may be empty (falls back to cheapest).
func NewAgentPromptCompressor(provider providers.Provider, model string) *AgentPromptCompressor {
	return &AgentPromptCompressor{inner: NewPromptCompressor(provider, model)}
}

func (a *AgentPromptCompressor) Compress(ctx context.Context, prompt string) (string, int, error) {
	out, res, err := a.inner.Compress(ctx, prompt)
	if err != nil || res == nil {
		return prompt, 0, err
	}
