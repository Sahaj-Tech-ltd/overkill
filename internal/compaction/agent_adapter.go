...
// escalation pipeline and returning the preserved trailing messages plus summary.
func (c *AgentCompactor) Compact(ctx context.Context, msgs []providers.Message, model string, maxTokens int) ([]providers.Message, string, error) {
	if c == nil || c.lcm == nil {
	...
	if res == nil {
		return nil, "", fmt.Errorf("compaction: nil result")
	}
	return res.Messages, res.Summary, nil
}
