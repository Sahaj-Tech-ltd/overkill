...
func (c *LCMCompactor) Compact(ctx context.Context, messages []providers.Message, opts CompactOptions) (*CompactionResult, error) {
	...
	if level == Level3Truncate && c.sink != nil {
		func() {
			defer func() {
				if r := recover(); r != nil {
					log.Printf("[compaction] panic in truncate sink: %v", r)
				}
			}()
...
func (c *LCMCompactor) compactLevels(ctx context.Context, messages []providers.Message, targetTokens int, model string) (string, Level, error) {
	...
	if err == nil && summary != "" && c.tokenizer.Estimate(summary) <= targetTokens {
...
func findSafeSplit(messages []providers.Message, splitIdx int) int {
	...
	// Floor: always preserve at least the first message (typically system
	// prompt) so the tail side is never completely empty even when every
	// subsequent message is a tool call/result pair.
	if splitIdx <= 1 {
		splitIdx = 1
	}
	return splitIdx
