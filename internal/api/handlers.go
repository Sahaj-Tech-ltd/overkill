...
func (s *Server) handleAgentSend(ctx context.Context, params []byte) (interface{}, *RPCError) {
	...
	// Snapshot config fields under lock to avoid data race with
	// concurrent config.update which writes these same fields.
	s.mu.RLock()
	creditProvider := s.cfg.Agent.DefaultProvider
	creditModel := s.cfg.Agent.DefaultModel
	s.mu.RUnlock()
	...
	s.recordCreditTurn(a, creditProvider, creditModel)
...
// configSnapshot builds a ConfigGetResult from the current config without
// acquiring any locks. The caller must hold s.mu (read or write).
func (s *Server) configSnapshot() *ConfigGetResult {
...
func (s *Server) handleConfigCreate(_ context.Context, params []byte) (interface{}, *RPCError) {
	...
	// Gateways.
	if p.Gateway != nil {
		if p.Gateway.Discord != nil {
			cfg.Gateways.Discord = config.DiscordConfig{
				Enabled:         p.Gateway.Discord.Enabled,
				BotToken:        p.Gateway.Discord.BotToken,
				NotifyChannelID: p.Gateway.Discord.NotifyChannelID,
			}
		}
		if p.Gateway.Telegram != nil {
			cfg.Gateways.Telegram = config.TelegramConfig{
				Enabled:      p.Gateway.Telegram.Enabled,
				BotToken:     p.Gateway.Telegram.BotToken,
				NotifyChatID: p.Gateway.Telegram.NotifyChatID,
			}
		}
		if p.Gateway.WhatsApp != nil {
			cfg.Gateways.WhatsApp = config.WhatsAppConfig{
				Enabled: p.Gateway.WhatsApp.Enabled,
				Backend: p.Gateway.WhatsApp.Backend,
			}
		}
	}
...
func (s *Server) createAgent(ctx context.Context, sessionID string) (*agent.Agent, error) {
	...
	if s.cfg.Security.AutonomyLevel != "" {
		a.SetAutoMode(s.cfg.Security.AutonomyLevel)
	}
