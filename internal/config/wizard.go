...
type WizardCatalog struct {
	...
	Review    []WizardOption `json:"review"`
}
...
// buildReviewCatalog returns provider options for Ouroboros/RedTeam
// adversarial review. These SHOULD be separate from the main agent —
// "don't grade your own homework." When the user picks the same provider
// as their main agent, we flag it with a warning tag.
func buildReviewCatalog() []WizardOption {
	return []WizardOption{
		{
			ID:          "deepseek",
			Name:        "DeepSeek (Review)",
			Description: "Cost-effective for test generation. Warning: using same provider as main agent means self-review. Pair with Anthropic for best results.",
			Rating:      RatingThree,
			Category:    CategoryReview,
			APIKeyEnv:   providers.CanonicalAPIKeyEnv("deepseek"),
			DefaultBase: providers.CanonicalBaseURL("deepseek"),
			RequiresKey: true,
			Tags:        []string{"cloud", "chinese", "self-review-warning"},
		},
		{
			ID:          "anthropic",
			Name:        "Anthropic Claude (Review)",
			Description: "Top-tier adversarial review. Different provider from DeepSeek — true independent audit. Best for catching bugs your main agent missed.",
			Rating:      RatingRecommended,
			Category:    CategoryReview,
			APIKeyEnv:   providers.CanonicalAPIKeyEnv("anthropic"),
			DefaultBase: providers.CanonicalBaseURL("anthropic"),
			...
			RequiresKey: true,
			Tags:        []string{"recommended", "cloud", "independent-review"},
		},
		{
			ID:          "openai",
			Name:        "OpenAI (Review)",
			Description: "Reliable review provider. GPT-4o for thorough audits, o3-mini for fast test generation. Different architecture from DeepSeek — good independent check.",
			Rating:      RatingThree,
			Category:    CategoryReview,
			APIKeyEnv:   providers.CanonicalAPIKeyEnv("openai"),
			DefaultBase: providers.CanonicalBaseURL("openai"),
			Models:      []string{"gpt-4o", "o3-mini"},
			RequiresKey: true,
			Tags:        []string{"cloud", "independent-review"},
		},
		{
			ID:          "openrouter",
			Name:        "OpenRouter (Review)",
			Description: "Multi-provider via one key. Can route Ouroboros to Claude and RedTeam to a cheap model. Good flexibility for audit pipelines.",
			Rating:      RatingTwo,
			Category:    CategoryReview,
			APIKeyEnv:   providers.CanonicalAPIKeyEnv("openrouter"),
			DefaultBase: providers.CanonicalBaseURL("openrouter"),
			RequiresKey: true,
			Tags:        []string{"cloud", "multi-provider", "flexible"},
		},
		{
			ID:          "same_as_main",
			Name:        "Same as Main Agent",
			Description: "Use the same provider as your main agent with a cheaper model. Cheapest option but weakest — the agent reviews its own code. Not recommended for production.",
			Rating:      RatingOne,
			Category:    CategoryReview,
			RequiresKey: false,
			Tags:        []string{"self-review-warning", "not-recommended"},
		},
	}
}
...
func (c WizardCatalog) RecommendedQuickSetup() QuickSetup {
	...
	if r := c.Recommend(CategoryReview); r != nil {
		qs.ReviewProvider = r.ID
		if len(r.Models) > 0 {
			qs.ReviewModel = r.Models[0]
		}
	}
...
func (c WizardCatalog) ApplyQuickSetup(cfg *Config) error {
	...
	// Wire review provider into Ouroboros and RedTeam configs.
	// The review provider is separate from the main agent — "don't grade your own homework."
	if qs.ReviewProvider != "" && qs.ReviewProvider != "same_as_main" {
		...
		cfg.RedTeam.Enabled = true
		cfg.RedTeam.Provider = qs.ReviewProvider
		if qs.ReviewModel != "" {
			cfg.RedTeam.Model = qs.ReviewModel
		}
	} else if qs.ReviewProvider == "same_as_main" && qs.Provider != "" {
		// User opted for self-review — use main provider with warning.
		cfg.Ouroboros.Enabled = true
		cfg.Ouroboros.Provider = qs.Provider
		// Use a cheaper model from the same provider if possible.
		cfg.RedTeam.Enabled = true
