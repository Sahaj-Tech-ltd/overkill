...
type Config struct {
	...
	RedTeam     RedTeamConfig     `toml:"red_team"`
...
type OuroborosConfig struct {
	...
	// SystemPrompt overrides the default adversarial reviewer prompt.
	// When empty, the built-in prompt is used.
	SystemPrompt string `toml:"system_prompt"`
}

// RedTeamConfig governs the red-team adversarial test generator
// (master plan §6.5, Wall 1 extension). Unlike Ouroboros (which
// reviews code for bugs), RedTeam writes and runs actual adversarial
// tests. The generator MUST use a separate provider/model from the
// build agent — otherwise it's the agent trying to break its own code.
// Off by default.
type RedTeamConfig struct {
	Enabled  bool   `toml:"enabled"`
	Provider string `toml:"provider"`  // e.g. "deepseek", "openai"
	Model    string `toml:"model"`     // model id on the test-gen provider
	APIKey   string `toml:"api_key"`   // falls back to <PROVIDER>_API_KEY
	BaseURL  string `toml:"base_url"`  // optional override
	MaxTests int    `toml:"max_tests"` // max tests per changed file (default 5)
	// SystemPrompt overrides the default adversarial test generator prompt.
	// When empty, the built-in prompt is used.
	SystemPrompt string `toml:"system_prompt"`
}
...
// SlackConfig governs the optional Slack bot gateway.
// Off by default; tokens may also come from SLACK_BOT_TOKEN /
// SLACK_APP_TOKEN at runtime.
// Supports Socket Mode (preferred — no public webhook URL needed)
// with fallback to Events API if Socket Mode is unavailable.
type SlackConfig struct {
	Enabled         bool     `toml:"enabled"`
	AppToken        string   `toml:"app_token"`        // xapp-... (Socket Mode)
	BotToken        string   `toml:"bot_token"`        // xoxb-... (Web API)
	AllowedUsers    []string `toml:"allowed_users"`    // empty = all users allowed
	...
	// notifications to (§7.1 Layer 6). Empty = no Slack push delivery.
...
type DiscordConfig struct {
	...
	// delivery.
	NotifyChannelID string `toml:"notify_channel_id"`
	// CutoffSec is the sliding window for Discord edit rate limiting
	// in seconds. Default 5.
	CutoffSec int `toml:"cutoff_sec"`
	// CleanupSec is the interval in seconds for cleaning up stale
	// edit rate limit entries. Default 600 (10 minutes).
	CleanupSec int `toml:"cleanup_sec"`
}
...
type WhatsAppCloudConfig struct {
	...
	// SeenTTLSec caps how long (in seconds) a seen message ID is
	// remembered for deduplication. Default 600 (10 minutes).
	SeenTTLSec int `toml:"seen_ttl_sec"`
}
...
type SignalConfig struct {
	...
	// NotifyNumber is the E.164 phone number the gateway pushes
	// unsolicited notifications to (§7.1 Layer 6). Empty = no Signal push delivery.
	NotifyNumber string `toml:"notify_number"`
	// SeenTTLSec caps how long (in seconds) a seen message hash is
	// remembered for deduplication. Default 600 (10 minutes).
	SeenTTLSec int `toml:"seen_ttl_sec"`
}
...
type MatrixConfig struct {
	...
	// NotifyRoomID is the Matrix room ID (e.g. "!abc123:homeserver")
	// the gateway pushes unsolicited notifications to (§7.1 Layer 6).
	// Empty = no Matrix push delivery.
	NotifyRoomID string `toml:"notify_room_id"`
	// SeenTTLSec caps how long (in seconds) a seen event ID is
	// remembered for deduplication. Default 600 (10 minutes).
	SeenTTLSec int `toml:"seen_ttl_sec"`
	// MemberCountTTLSec expires cached member counts so routing decisions
	// don't stay permanently stale after membership changes.
	// Default 300 (5 minutes).
	MemberCountTTLSec int `toml:"member_count_ttl_sec"`
}
...
type MattermostConfig struct {
	...
	// NotifyChannelID is the Mattermost channel ID the gateway pushes
	// unsolicited notifications to (§7.1 Layer 6). Empty = no Mattermost push delivery.
	NotifyChannelID string `toml:"notify_channel_id"`
}
...
type GatewayConfig struct {
	...
	// huge pastes. Default 100000.
	MaxTextLen int `toml:"max_text_len"`
	// MaxImageBytes caps total image bytes per message. Default 20000000.
	MaxImageBytes int `toml:"max_image_bytes"`
	// RateLimitPerMin is the rate limit per sender per minute. Default 10.
	RateLimitPerMin int `toml:"rate_limit_per_min"`
	// UpdateEveryMs is the batch reply edit interval in milliseconds. Default 750ms.
	UpdateEveryMs int `toml:"update_every_ms"`
	// BackoffInitialSec is the initial backoff in seconds for gateway hub
	// restarts. Default 1.
	BackoffInitialSec int `toml:"backoff_initial_sec"`
	// BackoffMaxSec is the maximum backoff cap in seconds for gateway hub
	// restarts. Default 30.
...
// CronConfig governs the cron scheduler's behaviour. Cron jobs produce
// output asynchronously; we hold that output until the agent is idle.
type CronConfig struct {
	// IdleWindowSec is how long (in seconds) the agent must be idle before
	// buffered cron outputs are flushed through the gateway.
	// Default 300 (5 minutes).
	IdleWindowSec int `toml:"idle_window_sec"`
}
...
type SyncConfig struct {
	...
	// PushTimeoutSec is the timeout in seconds for auto-push sync operations.
	// Default 30.
	PushTimeoutSec int `toml:"push_timeout_sec"`
}
...
type LSPConfig struct {
	...
	// MaxMessageBytes caps the maximum size of an LSP message in bytes.
	// Messages exceeding this cap are rejected to prevent OOM.
...
type ProviderConfig struct {
	...
	APIKey   string            `json:"apiKey,omitempty" toml:"api_key"`
	AuthType string            `json:"authType,omitempty" toml:"auth_type"`
	BaseURL  string            `json:"baseUrl,omitempty" toml:"base_url"`
	Models   []ModelConfig     `json:"models,omitempty" toml:"models"`
