...
type HistoryCompactor interface {
	Compact(ctx context.Context, msgs []providers.Message, model string, maxTokens int) (compactedMsgs []providers.Message, summary string, err error)
}
...
func (a *Agent) buildRequest() providers.Request {
	...
	if extra := a.providedContext(pctx); extra != "" {
		// B051: enforce a per-turn character budget on plugin context.
		// Without this, the combined output of skills + memory +
		// personality + context + goal can silently exceed the model's
		// context window, causing mid-prompt truncation.
		const maxContextChars = 4000
		if len(extra) > maxContextChars {
			extra = extra[:maxContextChars-20] + "\n\n*(truncated)*"
		}
...
func (a *Agent) ExportHistory(path string) (string, error) {
	...
	// Block writes to system paths — ExportHistory is a convenience tool,
	// not an arbitrary filesystem write primitive.
	...
	if strings.HasPrefix(cleaned, "/etc/") ||
		...
		strings.HasPrefix(cleaned, "/boot/") ||
		cleaned == "/etc" || cleaned == "/root" || cleaned == "/boot" {
		return "", fmt.Errorf("export: path %q is forbidden", cleaned)
	}
...
func (a *Agent) Compact(ctx context.Context) (*CompactResult, error) {
	...
	if useC && c != nil {
		...
		_, s, err := c.Compact(ctx, history, model, a.maxTokens)
		if err != nil {
...
func (a *Agent) reflectBeforeAction(input string) SituationalReflection {
	...
	user := UserModel{
		HasADHD:     false, // TODO: derive from personality engine when ADHD flag is available
		IsOnMobile:  false, // TODO: derive from gateway context (Telegram/WhatsApp/Signal/SMS)
