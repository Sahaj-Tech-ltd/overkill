...
func (s *SelfEvaluateLoop) RunPhase(
	...
	// Safely extract phase info — phase may be nil when called outside plan context.
	taskType := extractTaskType(phase)
	desc := "self-evaluate task"
	phaseTitle := ""
	if phase != nil {
	...
	for iter := 1; iter <= maxIter; iter++ {
		...
		// Execute the phase.
		history, err := runner(ctx, desc, revCtx)
		if err != nil {
		...
		case IterAccepted:
			...
			if redTeam != nil && redTeam.cfg.Enabled {
				workdir, _ := os.Getwd()
				rtResult, rtErr := redTeam.GenerateAndRun(ctx, allHistory, workdir, desc)
				if rtErr != nil {
					log.Warn().Err(rtErr).Str("phase", phaseTitle).Msg("self-evaluate: red-team check failed, continuing")
				} else if rtResult != nil && !rtResult.Passed {
					log.Warn().
						Str("phase", phaseTitle).
