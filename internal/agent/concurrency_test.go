...
// TestAgent_CompactorConcurrent tests SetCompactor under concurrent reads
// from the hot path (simulating checkBudget's read of useCompactor).
func TestAgent_CompactorConcurrent(t *testing.T) {
	prov := &mockProvider{
		responses: []providers.Response{{Content: "test"}},
	}
	a := New(Config{
		Provider:  prov,
	...
	var wg sync.WaitGroup
	// G1: frequently set/unset compactor.
	for i := 0; i < 30; i++ {
		wg.Add(1)
		go func(id int) {
			defer wg.Done()
			if id%2 == 0 {
				a.SetCompactor(nil, false)
			} else {
				a.SetCompactor(&fakeCompactor{out: "summary"}, true)
			}
		}(i)
	}
	// G2: read checkBudget path (reads useCompactor atomically).
	for i := 0; i < 30; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
