...
type Loader struct {
	...
	// #104: injectionBlocker scans parsed skill instructions for
	// prompt-injection patterns before the skill enters the registry.
	// nil blocker → no injection scanning (preserves previous behavior).
	injectionBlocker func(instructions string) error
...
// WithInjectionBlocker installs a function that scans parsed skill
// instruction text for prompt-injection patterns. When non-nil, it is
// called after parsing but before the skill enters the registry.
// A non-nil error blocks the skill. #104.
func (l *Loader) WithInjectionBlocker(fn func(instructions string) error) *Loader {
	l.injectionBlocker = fn
	return l
}
...
func (l *Loader) LoadFile(path string) (*Skill, error) {
	// #103: Stat first to check file size before reading into memory.
	// A maliciously large SKILL.md can cause OOM.
	info, err := os.Stat(path)
	if err != nil {
		return nil, fmt.Errorf("skills: stating file %q: %w", path, err)
	}
	const maxSkillSize = 1 << 20 // 1 MB
	if info.Size() > maxSkillSize {
		return nil, fmt.Errorf("skills: file %q too large (%d bytes, limit %d)", path, info.Size(), maxSkillSize)
	}
	...
	// #104: Scan parsed instructions for prompt-injection patterns.
	if l.injectionBlocker != nil {
		if err := l.injectionBlocker(skill.Instructions); err != nil {
			return nil, fmt.Errorf("skills: injection blocked in %q: %w", path, err)
		}
	}
