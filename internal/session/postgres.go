...
func (s *PostgresStore) migrate() error {
	cmds := []string{
		...
		`CREATE TABLE IF NOT EXISTS session_follows (
...
func (s *PostgresStore) List(ctx context.Context, opts ListOptions) ([]*Session, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&sess.ID, &sess.Title, &sess.Folder, &sess.ParentID,
...
func (s *PostgresStore) Merge(ctx context.Context, childID string) (*Session, error) {
	...
	result, err := tx.ExecContext(ctx, `
		UPDATE sessions
		SET messages = $1, turn_count = $2, updated_at = $3
		WHERE id = $4`,
...
func (s *PostgresStore) Bind(channel, chatKey, thread, sessionID string) error {
	_, err := s.db.Exec(`
...
func (s *PostgresStore) Resolve(channel, chatKey, thread, liveSessionID string) (string, error) {
...
// clears follow mode.
func (s *PostgresStore) Follow(chatKey, target string) error {
	// Update all entries for this chat_key.
	_, err := s.db.Exec(`
...
func (s *PostgresStore) Touch(channel, chatKey, thread string) error {
	_, err := s.db.Exec(`
...
func (s *PostgresStore) Recent(limit int) ([]RouterRow, error) {
...
func (s *PostgresStore) Unbind(channel, chatKey, thread string) error {
	_, err := s.db.Exec(`
