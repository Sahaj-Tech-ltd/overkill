...
type ReadCache struct {
	mu       sync.Mutex
	entries  map[string]*list.Element // value is *Entry
	order    *list.List               // LRU order, oldest at front
	maxBytes int64
	curBytes int64
	ttl      time.Duration
...
func (c *ReadCache) Put(path string, bytes []byte, mtime time.Time) {
	...
	if existing, ok := c.entries[path]; ok {
		ex := existing.Value.(*Entry)
		c.curBytes -= int64(len(ex.Bytes))
		c.order.Remove(existing) // remove old list element to prevent orphan
	}
	e := &Entry{
		Path:     path,
	...
	c.entries[path] = c.order.PushBack(e)
	c.curBytes += int64(len(bytes))
	// Already at back, no need to touch.
	c.evictExcessLocked()
}
...
type Prefetcher struct {
	mu      sync.Mutex // protects stop channel lifecycle (Start/Stop)
	cache   *ReadCache
	queue   chan string
	wg      sync.WaitGroup
	stop    chan struct{}
