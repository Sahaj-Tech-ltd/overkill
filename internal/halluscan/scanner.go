...
func (s *Scanner) Scan(content, evidence string) Result {
	...
	// Build the flagged list from only the capped hits so it matches
	// the annotated output (no more entries than markers inserted).
	seen := make(map[string]bool, cap)
	uniq := make([]string, 0, cap)
	for _, h := range hits {
		if seen[h.id] {
			continue
		}
		seen[h.id] = true
		uniq = append(uniq, h.id)
	}
