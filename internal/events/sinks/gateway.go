...
// injected SendFunc.  The context parameter is accepted to satisfy the
// events.Sink interface but is intentionally unused — SendFunc's signature
// (channel, chatKey, text) does not accept a context, and the bridge's SSE
// fanout is fire-and-forget by design.  If the bridge ever grows a
// context-aware send variant, wire it here.
