...
// NewStore wraps a *sql.DB for PostgreSQL-backed correction storage.
// The caller owns the DB lifecycle — the store does not close it.
func NewStore(db *sql.DB, maxItems int) (*Store, error) {
	if maxItems <= 0 {
...
func (s *Store) Save(c *Correction) error {
	...
	tx, err := s.db.Begin()
	if err != nil {
		return fmt.Errorf("learning: begin tx: %w", err)
	}
	defer tx.Rollback()
	...
	var count int
	if err := tx.QueryRow("SELECT COUNT(*) FROM corrections").Scan(&count); err != nil {
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("learning: commit eviction: %w", err)
	}
...
func (s *Store) FindCorrections(query string, topK int) ([]*Correction, error) {
	...
	// Cap tokens to prevent unbounded bind parameters.
	const maxTokens = 20
	if len(queryTokens) > maxTokens {
		queryTokens = queryTokens[:maxTokens]
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.Context, &c.Wrong, &c.Correct, &c.Timestamp); err != nil {
			log.Printf("[learning] scan error: %v", err)
			continue
		}
