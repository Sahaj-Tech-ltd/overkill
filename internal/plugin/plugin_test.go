...
func TestAllowsToolWildcard(t *testing.T) {
	...
	if p.AllowsTool("anything") {
		t.Fatal("wildcard should NOT allow arbitrary tools — removed for security (C2 fix)")
	}
}
...
func TestAllowsEventWildcard(t *testing.T) {
	...
	if p.AllowsEvent("any_event") {
		t.Fatal("wildcard should NOT allow arbitrary events — removed for security (C2 fix)")
	}
}
...
func TestConcurrentRegistration(t *testing.T) {
	plug := &fakePlugin{
		manifest: Manifest{
			Name:    "concur",
			Version: "0.1.0",
			Permissions: Permissions{Events: []string{"ev-0", "ev-1", "ev-2", "ev-3", "ev-4",
				"ev-5", "ev-6", "ev-7", "ev-8", "ev-9",
				"ev-10", "ev-11", "ev-12", "ev-13", "ev-14",
				"ev-15", "ev-16", "ev-17", "ev-18", "ev-19"}},
		},
	}
	fx := newFixture(t, plug)
	defer fx.close()
