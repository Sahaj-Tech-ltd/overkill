...
func discoverDir(dir, name string) (*Discovered, error) {
	...
	if data, err := os.ReadFile(tomlPath); err == nil {
		...
		// B111 / #55: Resolve symlinks before containment check.
		// filepath.Abs does NOT follow symlinks, so a symlinked entry
		// inside the plugins dir can point outside and pass the lexical
		// HasPrefix check. EvalSymlinks resolves to the real path.
		// Non-existent entries are allowed — plugins may create files at
		// init time (EvalSymlinks raises os.ErrNotExist for these).
		resolvedEntry, entryErr := filepath.EvalSymlinks(absEntry)
		if entryErr != nil && !os.IsNotExist(entryErr) {
			return nil, fmt.Errorf("plugin: resolve entry symlinks %q: %w", entry, entryErr)
		}
		resolvedDir, dirErr := filepath.EvalSymlinks(absDir)
		if dirErr != nil {
			return nil, fmt.Errorf("plugin: resolve dir symlinks: %w", dirErr)
		}
		if entryErr == nil && !strings.HasPrefix(resolvedEntry, resolvedDir+string(filepath.Separator)) {
