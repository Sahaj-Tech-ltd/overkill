// Package cron — timezone-aware cron scheduler with Postgres-backed jobs.
// Wired via daemon.go; busy-gate + output buffer prevent message storms.
package cron
...
type Job struct {
	...
	// BackoffBaseSec is the initial retry delay in seconds (default 60).
	// Doubles on each subsequent failure up to BackoffMaxSec.
	BackoffBaseSec int `json:"backoff_base_sec"`
	// BackoffMaxSec caps the retry delay (default 300).
