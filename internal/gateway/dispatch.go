...
var reShellShortcut = regexp.MustCompile(`^\$(\s|shell\s)`)
...
type Dispatcher struct {
	...
	// MaxTextLen caps text per inbound message. 0 means use default (100000).
	MaxTextLen int
	// MaxImageBytes caps total image bytes per message. 0 means use default (20000000).
	MaxImageBytes int
	// MaxReqsPerMinute is the rate limit per sender per minute. 0 means use default (10).
	...
	// agentMu guards reads/writes to Agent.SessionID() / Agent.SetSessionID()
	// so that resolveSession (reader) and runTurn (writer) don't TOCTOU
	// across different striped locks.
	agentMu sync.Mutex
...
// ApplyGatewayLimits sets dispatch limits from a GatewayConfig. Zero
// values are ignored (the built-in defaults remain).
func (d *Dispatcher) ApplyGatewayLimits(cfg GatewayLimitsCfg) {
	if cfg.MaxTextLen > 0 {
		d.MaxTextLen = cfg.MaxTextLen
	}
	if cfg.MaxImageBytes > 0 {
		d.MaxImageBytes = cfg.MaxImageBytes
	}
	if cfg.RateLimitPerMin > 0 {
		d.MaxReqsPerMinute = cfg.RateLimitPerMin
	}
	if cfg.UpdateEveryMs > 0 {
		d.UpdateEvery = time.Duration(cfg.UpdateEveryMs) * time.Millisecond
	}
}

// GatewayLimitsCfg is a minimal interface for passing dispatch limits
// without importing config.
type GatewayLimitsCfg struct {
	MaxTextLen      int
	MaxImageBytes   int
	RateLimitPerMin int
	UpdateEveryMs   int
...
func (d *Dispatcher) Handle(ctx context.Context, in Inbound, reply Reply) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("gateway: panic in Handle from=%s: %v", in.From, r)
		}
	}()
	...
	// H-17: Rate limiting — per-sender rate limit.
	...
	if len(text) > maxTL {
		text = TruncateMessage(text, maxTL)
	}
	var totalImageBytes int64
	for _, img := range in.Images {
		totalImageBytes += int64(len(img.Bytes))
	}
	maxIB := d.MaxImageBytes
	if maxIB <= 0 {
		maxIB = 20_000_000
	}
	if totalImageBytes > int64(maxIB) {
		d.Logger.Printf("dispatch: image size %d exceeds limit %d", totalImageBytes, maxIB)
		return
	}

	// --- Pre-pairing allowlist ---
	// These slash commands MUST work before pairing approval so users
	// can create sessions, attach, and receive help during the challenge
	// flow. Security: none of these execute code or mutate state outside
	// the session router.
	if text != "" && strings.HasPrefix(text, "/") && len(in.Images) == 0 {
		cmd, _ := splitCommand(text)
		if d.isPrePairingCommand(cmd) {
	...
	// --- Pairing gate ---
	// Check sender approval before processing any command or message,
	// including $shell shortcuts and non-allowlisted slash commands.
	// Unknown senders receive a pairing challenge.
	if d.Pairing != nil && in.From != "" && !d.Pairing.IsApproved(in.Channel, in.From) {
		d.Logger.Printf("dispatch: unapproved sender %q — issuing challenge", in.From)
		go func() {
			defer func() {
				if r := recover(); r != nil {
					d.Logger.Printf("dispatch: panic in pairing challenge: %v", r)
				}
			}()
			code, err := d.Pairing.IssueChallenge(in.Channel, in.From)
			if err != nil {
				d.Logger.Printf("dispatch: challenge issue failed: %v", err)
				return
			}
			d.respond(context.Background(), in, reply, fmt.Sprintf(
				"🔐 New sender — approval required.\n\nYour pairing code: **%s**\n\nAsk the bot owner to run: `overkill pairing approve %s %s`",
				code, in.Channel, code,
			))
		}()
		return
	}
	...
	// $shell shortcut — direct command execution, no agent loop.
	// Works from any gateway: Telegram, Discord, WhatsApp, etc.
	// "$ ls -la" or "$shell git status" → executes immediately.
	if text != "" && reShellShortcut.MatchString(text) && len(in.Images) == 0 {
		d.handleShellShortcut(ctx, in, reply, text)
		return
	}

	// Slash commands — fast-path for the approved sender.
	if text != "" && strings.HasPrefix(text, "/") && len(in.Images) == 0 {
		d.handleCommand(ctx, in, reply, text)
		return
	}
	...
	if d.active[sid] {
		...
		if queueLen == 1 {
			...
			d.respond(context.Background(), in, reply, fmt.Sprintf("📥 You're %s in line — agent is still working on turn %d.", ordinal(queueLen), queueLen))
		}
		return
	}
	d.active[sid] = true
	...
	if d.MsgDelivery != nil {
		undelivered, err := d.MsgDelivery.GetUndelivered(ctx, sid)
		if err != nil {
			d.Logger.Printf("dispatch: get undelivered: %v", err)
		}
		if len(undelivered) > 0 {
			...
			for _, um := range undelivered {
				...
				retryID, err := d.MsgDelivery.RecordPending(ctx, sid, in.ChatKey, um.Message+" [auto-retry]")
				if err != nil {
					d.Logger.Printf("dispatch: record pending (retry): %v", err)
				}
				go func() {
					defer func() {
						if r := recover(); r != nil {
							d.Logger.Printf("dispatch: panic in auto-retry undelivered: %v", r)
						}
					}()
					// Guard against concurrent turns on same session.
					d.activeMu.Lock()
					if d.active[sid] {
						d.activeMu.Unlock()
						return
					...
					d.runTurn(context.Background(), in, reply, sid, um.Message, retryID)
				}()
			}
		}
	}
	...
	var recordErr error
	if d.MsgDelivery != nil {
		deliveryID, recordErr = d.MsgDelivery.RecordPending(ctx, sid, in.ChatKey, prompt)
		if recordErr != nil {
			d.Logger.Printf("dispatch: record pending: %v", recordErr)
		}
	}
	...
	if len(d.pending[sid]) > 0 {
		...
		// Loop through ALL queued messages for this session (sequential).
		go func() {
			defer func() {
				if r := recover(); r != nil {
					d.Logger.Printf("dispatch: panic in queued auto-fire: %v", r)
				}
			}()
			for {
				d.activeMu.Lock()
				...
				d.runTurn(context.Background(), in, reply, sid, nextPrompt, 0)

				// Clean up active flag and check for next queued message.
				d.activeMu.Lock()
				delete(d.active, sid)
				d.pendingMu.Lock()
				if len(d.pending[sid]) > 0 {
...
// effectiveMaxReqs returns the configured per-minute rate limit, falling
// back to 10 if unset or zero.
func (d *Dispatcher) effectiveMaxReqs() int {
	if d.MaxReqsPerMinute <= 0 {
		return 10
	}
	return d.MaxReqsPerMinute
}
...
func (d *Dispatcher) checkRateLimit(from string) bool {
	...
	// Prune empty entries to prevent unbounded map growth over time.
	// If all timestamps expired, the sender is not rate-limited —
	// don't re-store the empty slice; it'll be re-created on next message.
	if len(timestamps) == 0 {
		delete(d.rateLimits, from)
		return true
	}

	if len(timestamps) >= d.effectiveMaxReqs() {
...
func (d *Dispatcher) buildPrompt(ctx context.Context, in Inbound, text string) string {
	...
	for i, img := range in.Images {
		...
		go func(idx int, img InboundImage) {
			defer func() {
				if r := recover(); r != nil {
					d.Logger.Printf("dispatch: panic in vision describe: %v", r)
				}
			}()
			defer wg.Done()
...
// Holds agentMu throughout to prevent TOCTOU: a concurrent runTurn may
// call SetSessionID on a different striped lock, changing the value
// between our SessionID() read and the Router.Resolve call that uses it.
...
func (d *Dispatcher) runTurn(ctx context.Context, in Inbound, reply Reply, sessionID, text string, deliveryID int64) {
	...
	var stream <-chan agent.StreamEvent
	var streamErr error
	d.serialize(sessionID, func() {
		if d.Agent == nil {
			streamErr = fmt.Errorf("no agent configured")
			return
		}
		d.agentMu.Lock()
		d.Agent.SetSessionID(sessionID)
		d.agentMu.Unlock()
		stream, streamErr = d.Agent.Stream(ctx, text)
	})
	if streamErr != nil {
		if replyErr := reply.Error(ctx, handle, streamErr); replyErr != nil {
			d.Logger.Printf("dispatch: error reply: %v", replyErr)
		}
		if deliveryID > 0 && d.MsgDelivery != nil {
			d.MsgDelivery.MarkFailed(ctx, deliveryID, streamErr.Error())
		}
		return
	}
...
func (d *Dispatcher) streamInto(ctx context.Context, reply Reply, handle string, stream <-chan agent.StreamEvent) {
	...
	flush := func(force bool) {
		...
		mu.Lock()
		lastSent = current
		// Only clear dirty if no new content arrived while we were sending.
		if buf.String() == lastSent {
			dirty = false
		}
		mu.Unlock()
	}
	...
	// Note: UpdateEvery is always > 0 (default 750ms, config override only
	// accepts positive values), so the zero-guard is unnecessary.
	defer tick.Stop()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				d.Logger.Printf("dispatch: panic in streamInto: %v", r)
			}
			close(done)
		}()
		for {
	...
	for {
		...
		case <-done:
			...
			mu.Unlock()
			if err != nil {
				if replyErr := reply.Error(ctx, handle, err); replyErr != nil {
					d.Logger.Printf("dispatch: stream error reply: %v", replyErr)
				}
				return
			}
			if final == "" {
			...
			fctx, cancel := context.WithTimeout(ctx, 10*time.Second)
			if err := reply.Final(fctx, handle, final); err != nil {
				d.Logger.Printf("dispatch: final: %v", err)
			}
			cancel()
			return
		}
...
// handleShellShortcut runs a bare command directly, bypassing the agent loop.
// Triggered by \"$ cmd\" or \"$shell cmd\" from any gateway — same shortcut the
// TUI offers. Output is streamed back as a code block. Timeout matches the
// TUI default (15 min). Commands inherit the daemon's working directory and
// environment — the agent's workspace isolation does not apply here.
func (d *Dispatcher) handleShellShortcut(ctx context.Context, in Inbound, reply Reply, raw string) {
	cmdStr := reShellShortcut.ReplaceAllString(raw, "")
	cmdStr = strings.TrimSpace(cmdStr)
	if cmdStr == "" {
		d.respond(ctx, in, reply, "usage: `$ <command>` — e.g. `$ ls -la` or `$ git status`")
		return
	}

	d.Logger.Printf("dispatch: $shell from %s/%s: %s", in.Channel, in.From, cmdStr)

	timeoutCtx, cancel := context.WithTimeout(ctx, 15*time.Minute)
	defer cancel()

	shellCmd := exec.CommandContext(timeoutCtx, "sh", "-c", cmdStr)
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			d.respond(ctx, in, reply, b.String())
			return
		}
	}

	if exitCode != 0 {
		b.WriteString(fmt.Sprintf("❌ exit %d  ⏱ %s\n", exitCode, elapsed.Round(time.Millisecond)))
	} else {
...
func (d *Dispatcher) handleCommand(ctx context.Context, in Inbound, reply Reply, raw string) {
	...
	case "/stop":
		...
		d.Agent.Interrupt()
		if queueLen > 0 {
			d.respond(context.Background(), in, reply, fmt.Sprintf("⏸️ stopped. %d message(s) queued.", queueLen))
		} else {
	...
	case "/retry":
		...
		go func() {
			defer func() {
				if r := recover(); r != nil {
					d.Logger.Printf("dispatch: panic in async retry: %v", r)
				}
			}()
			res, err := d.Agent.Retry(context.Background())
			if err != nil {
	...
	case "/fork":
		...
		d.respond(context.Background(), in, reply, fmt.Sprintf("🔀 forked to new session: %s (%s)", newSID, name))
	case "/goal":
		...
		if arg == "" {
			...
			d.respond(context.Background(), in, reply, fmt.Sprintf("🎯 current goal: %s", current))
			return
		}
		...
		case "set":
			...
			d.respond(context.Background(), in, reply, fmt.Sprintf("🎯 goal set: %s", goalText))
		case "pause":
			if d.Agent == nil {
				d.respond(ctx, in, reply, "agent not available")
				return
	...
	case "/snapshot":
		...
		d.respond(context.Background(), in, reply, fmt.Sprintf("📸 snapshot created: %s (%s)", name, hash))
	case "/rollback":
		n := 0
		if arg != "" {
			fmt.Sscanf(arg, "%d", &n)
	...
	case "/snapshots":
		...
		d.respond(ctx, in, reply, result)
	case "/compact":
		if d.busyGuard(ctx, in, reply, "compact") {
			return
		}
		if d.Agent == nil {
			d.respond(ctx, in, reply, "agent not available")
		...
		result, err := d.Agent.Compact(ctx)
		if err != nil {
			d.respond(ctx, in, reply, "compact failed: "+err.Error())
			return
		}
		if result.TokensBefore == 0 {
			d.respond(ctx, in, reply, "🗜️ Nothing to compact — history is already minimal.")
			return
		}
		saved := 0
		if result.TokensBefore > 0 {
		...
		d.respond(ctx, in, reply, fmt.Sprintf(
			"🗜️ Compacted: %d → %d tokens (%d%% saved).\n%s",
			result.TokensBefore, result.TokensAfter, saved, result.Summary,
		))
	case "/think":
		if d.busyGuard(ctx, in, reply, "think") {
			return
		}
		if d.Agent == nil {
			d.respond(ctx, in, reply, "agent not available")
		...
		current := d.Agent.ThinkingLevel()
		if current == "" || current == "off" {
			d.Agent.SetThinkingLevel("on")
			d.respond(ctx, in, reply, "🧠 Thinking on — preambles will stream before tool calls.")
		} else {
			d.Agent.SetThinkingLevel("off")
			d.respond(ctx, in, reply, "🧠 Thinking off — preamble streaming disabled.")
		}
	case "/plan":
		// Delegate to agent stream; StreamWithAttachments parses and
		// runs DeepPlanner synchronously via handleSlashCommand.
		sid := d.resolveSession(in)
		d.runTurn(ctx, in, reply, sid, raw, 0)
		return
	case "/build":
		// Delegate to agent stream; StreamWithAttachments parses and
		// activates build mode synchronously via handleSlashCommand.
		sid := d.resolveSession(in)
		d.runTurn(ctx, in, reply, sid, raw, 0)
		return
	case "/usage":
...
//
// NOTE: This calls PostInitial then immediately Final. For edit-capable
// channels (Telegram, Discord, Slack) this produces a clean single message
// via edit. For non-edit channels it may double-send — but respond() is
// only used by slash commands, and all currently supported channels handle
// editing. If a non-edit channel is added, this needs revisiting.
...
func helpText() string {
	return strings.Join([]string{
		...
		"  $ <cmd>           run a shell command directly (e.g. $ ls, $ git status)",
