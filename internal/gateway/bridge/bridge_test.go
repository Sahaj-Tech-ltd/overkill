...
func (f *fakeAgent) Compact(_ context.Context) (*agent.CompactResult, error) {
	return &agent.CompactResult{TokensBefore: 100, TokensAfter: 50}, nil
}
...
func TestSubscribeUnsubscribe(t *testing.T) {
	...
	s := b.subs["test-channel"][id]
	doneCh := s.done
	b.mu.Unlock()

	b.unsubscribe("test-channel", id)

	// done channel should be closed after unsubscribe; ch is NOT closed
	// (prevents send-on-closed-channel panic in emit).
	select {
	case _, ok := <-doneCh:
		if ok {
	...
	default:
		t.Error("done channel should be closed, not blocking")
	}

	// ch should still be open (not closed) — verify by checking it
	// does NOT return zero-value immediately.
	select {
	case _, ok := <-ch:
		if !ok {
			// Channel IS closed — this is unexpected with the new design
			t.Error("ch should NOT be closed after unsubscribe")
		}
	default:
		// Good: channel is still open, not closed
	}
}
...
func TestEmitDropsSlowSubscriber(t *testing.T) {
	...
	if b.subs["bridge:slow"] == nil {
		b.subs["bridge:slow"] = map[int64]*subscriber{}
	}
	b.subs["bridge:slow"][id] = s
	b.mu.Unlock()
...
// TestBridge_EmitUnsubscribeRace exercises the race between emit() and
// unsubscribe(). Bug #33: emit held channel refs after releasing the
// mutex; if unsubscribe() closed the channel between the unlock and
// the send, emit would panic on send-to-closed-channel.
func TestBridge_EmitUnsubscribeRace(t *testing.T) {
	b := New(nil, "", "")
	id, ch := b.subscribe("bridge:race-test")

	// Get the done channel for the drain goroutine.
	b.mu.Lock()
	s := b.subs["bridge:race-test"][id]
	doneCh := s.done
	b.mu.Unlock()

	// Spawn a goroutine that drains the channel so emit doesn't drop.
	go func() {
		for {
			select {
			case <-doneCh:
	...
	go func() {
		// Rapidly emit and unsubscribe in a tight loop to trigger
		// the race between the emit goroutine's channel send and
		// the unsubscribe goroutine's channel close.
		for i := 0; i < 1000; i++ {
			// Each iteration: subscribe a fresh sub, then race emit vs unsubscribe.
			id, _ := b.subscribe("bridge:race-test")
			go func(sid int64) {
				b.emit(OutboundFrame{Channel: "bridge:race-test", Kind: "post", Text: "x"})
			}(id)
			b.unsubscribe("bridge:race-test", id)
		}
		close(done)
	}()
