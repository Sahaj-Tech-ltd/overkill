...
type InputHistory struct {
	db         *sql.DB
	maxEntries int
	mu         sync.Mutex // serializes read-modify-write in Append
}
...
func (ih *InputHistory) Append(_ context.Context, chatKey, text string) error {
	ih.mu.Lock()
	defer ih.mu.Unlock()

	// Read current entries
	var entries []string
	err := ih.db.QueryRow(`SELECT messages FROM input_history WHERE chat_key = $1`, chatKey).Scan(&entries)
	if err == sql.ErrNoRows {
		entries = []string{}
	} else if err != nil {
		// Try to scan as raw bytes if the driver didn't handle []string
		var raw []byte
		err2 := ih.db.QueryRow(`SELECT messages FROM input_history WHERE chat_key = $1`, chatKey).Scan(&raw)
		if err2 != nil {
			entries = []string{}
		} else {
