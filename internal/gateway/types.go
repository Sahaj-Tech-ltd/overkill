...
type AgentSender interface {
	...
	Retry(ctx context.Context) (string, error)
...
// Notifier is implemented by Channel bots that can send unsolicited
// messages to a target (channel, chat, JID) without a prior inbound
// message. Used by the §7.1 Layer 6 completion-push poller and other
// cron/alert systems to deliver notifications through already-open
// gateway connections.
type Notifier interface {
	Notify(ctx context.Context, target string, text string) error
}
