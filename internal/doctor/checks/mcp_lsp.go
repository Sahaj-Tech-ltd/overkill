		...
		r.Register(doctor.SubsystemCheck{
			...
			Fn: func(ctx context.Context) doctor.Result {
				...
				if _, err := c.ListTools(ctx); err != nil {
					return warnf(fmt.Sprintf("ListTools failed: %v — check that the MCP server process is running and the socket/URL is reachable", err),
						"ListTools: %v", err)
				}
	...
	for _, p := range commonLSPProbes {
		...
		r.Register(doctor.SubsystemCheck{
			...
			Fn: func(ctx context.Context) doctor.Result {
				...
				c := lsp.NewClient(p.Lang, p.Cmd, nil, 0)
				if err := c.Start(ctx, "."); err != nil {
					// Don't defer c.Close() on a failed Start — the client
					// may be partially initialized and Close could panic.
