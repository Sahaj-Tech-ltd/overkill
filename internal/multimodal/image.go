...
type ImageExtractor struct {
	...
	// MaxBytes caps the image file size before reading. Default 10MB (B010).
	MaxBytes int64
}
...
func NewImageExtractor(d vision.Describer) *ImageExtractor {
	return &ImageExtractor{Describer: d, Timeout: 30 * time.Second, MaxBytes: 10 << 20}
}
...
func (e *ImageExtractor) Extract(ctx context.Context, path string) (Result, error) {
	// B010: check file size before reading into memory.
	if e.MaxBytes > 0 {
		fi, err := os.Stat(path)
		if err != nil {
			return Result{}, fmt.Errorf("image: stat %s: %w", path, err)
		}
		if fi.Size() > e.MaxBytes {
			return Result{}, fmt.Errorf("image: %s too large (%d bytes, max %d)", path, fi.Size(), e.MaxBytes)
		}
	}
