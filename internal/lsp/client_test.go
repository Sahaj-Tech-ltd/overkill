...
func newConnectedFakeClient(t *testing.T, ctx context.Context) (*Client, func()) {
	...
	c.conn = newJSONRPCConn(c2sW, s2cR, 0)
...
func TestJSONRPCConn_ClosedOperations(t *testing.T) {
	t.Run("CallOnClosed", func(t *testing.T) {
		c := newJSONRPCConn(io.Discard, strings.NewReader(""), 0)
		c.close()
...
func TestJSONRPCConn_DispatchNonNumericID(t *testing.T) {
	...
	c := newJSONRPCConn(io.Discard, s2cR, 0)
...
func TestReadLoop_ContentLengthZero(t *testing.T) {
	r, w := io.Pipe()
	c := newJSONRPCConn(io.Discard, r, 0)
	go func() {
...
func TestNewClient(t *testing.T) {
	c := NewClient("go", "gopls", []string{"serve"}, 0)
	if c.Language() != "go" {
...
func TestClient_NullResults(t *testing.T) {
	...
	t.Run("DocumentSymbols_null", func(t *testing.T) {
		...
		cl.conn = newJSONRPCConn(inW, outR, 0)
...
// TestClient_ValidatePath verifies workspace containment checks (#176).
// LSP methods must reject queries for files outside the workspace root
// to prevent exfiltration of sensitive files like /etc/shadow.
func TestClient_ValidatePath(t *testing.T) {
	// Client with workspace root at /tmp
	c := &Client{rootURI: "file:///tmp"}
	...
	tests := []struct {
		...
		{"inside_root", "/tmp/main.go", false},
		{"inside_root_subdir", "/tmp/pkg/foo.go", false},
		{"outside_root_etc", "/etc/shadow", true},
		{"outside_root_home", "/home/user/.ssh/id_rsa", true},
		{"outside_root_var", "/var/log/auth.log", true},
		{"root_itself", "/tmp", false},
		{"relative_inside", "main.go", false}, // relative paths resolved to abs
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := c.validatePath(tt.file)
			if tt.wantErr && err == nil {
				t.Errorf("expected error for %q but got nil", tt.file)
			}
			if !tt.wantErr && err != nil {
	...
	// Test with unset rootURI
	c2 := &Client{}
	err := c2.validatePath("/tmp/x.go")
	if err != nil {
		t.Logf("unset rootURI: %v (expected pass or soft error)", err)
	}
}

// TestClient_MethodsRejectOutsideWorkspace verifies that LSP query methods
// (Definition, References, Hover, DocumentSymbols) reject paths outside
// the workspace root.
func TestClient_MethodsRejectOutsideWorkspace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	...
	// Override the rootURI to a specific path
	cl.rootURI = "file:///tmp"
	...
	for _, path := range outsidePaths {
		// Each method should error, not call the LSP server
		_, err := cl.Definition(ctx, path, 0, 0)
		if err == nil {
			t.Errorf("Definition(%q): expected workspace containment error", path)
		} else {
			t.Logf("Definition(%q): correctly rejected: %v", path, err)
		}

		_, err = cl.References(ctx, path, 0, 0)
		if err == nil {
			t.Errorf("References(%q): expected workspace containment error", path)
		}

		_, err = cl.Hover(ctx, path, 0, 0)
		if err == nil {
			t.Errorf("Hover(%q): expected workspace containment error", path)
		}

		_, err = cl.DocumentSymbols(ctx, path)
		if err == nil {
			t.Errorf("DocumentSymbols(%q): expected workspace containment error", path)
		}
	}
}
