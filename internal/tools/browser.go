...
func (t *BrowserEvalTool) Execute(ctx context.Context, in json.RawMessage) (json.RawMessage, error) {
	...
	if exfil := scanJSForExfil(args.JS); exfil != "" {
		return nil, fmt.Errorf("browser_eval: blocked — %s", exfil)
	}
	b, err := t.ensureOpen(ctx)
	if err != nil {
		return nil, err
...
// scanJSForExfil checks JavaScript for data-exfiltration patterns and
// returns a human description of the first blocked pattern, or "" if
// the JS is safe to evaluate. This is a surface-level scan — not a
// full JS parser — targeting the most common exfiltration vectors.
var jsExfilPatterns = []struct {
	re   *regexp.Regexp
	desc string
}{
	{regexp.MustCompile(`\bfetch\s*\(`), "network request (fetch)"},
	{regexp.MustCompile(`\bXMLHttpRequest\b`), "network request (XHR)"},
	{regexp.MustCompile(`\bdocument\.cookie\b`), "cookie access"},
	{regexp.MustCompile(`\blocalStorage\b`), "storage access (localStorage)"},
	{regexp.MustCompile(`\bsessionStorage\b`), "storage access (sessionStorage)"},
	{regexp.MustCompile(`\bnavigator\.sendBeacon\b`), "beacon exfiltration"},
	{regexp.MustCompile(`\blocation\s*[:.]\s*href\s*=`), "URL redirect"},
	{regexp.MustCompile(`\bwindow\.open\s*\(`), "window.open (popup exfil)"},
	{regexp.MustCompile(`\bimport\s*\(`), "dynamic import"},
}

func scanJSForExfil(js string) string {
	for _, p := range jsExfilPatterns {
		if p.re.MatchString(js) {
			return p.desc
		}
	}
	return ""
