...
func ovenNotifySlack(ctx context.Context, a config.BoardOvenAction, info TaskInfo) error {
	...
	cmd := exec.CommandContext(ctx, "curl", "-s", "-X", "POST", "-H", "Content-Type: application/json",
		"--data", string(payloadBytes), webhookURL)
	out, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("notify_slack: curl: %s: %w", strings.TrimSpace(string(out)), err)
	}
	return nil
}
