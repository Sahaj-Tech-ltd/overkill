...
func (s *PostgresSOPStore) LoadSOPs() ([]SOP, error) {
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(stepsJSON, &sop.Steps); err != nil {
			log.Printf("automation: unmarshal steps for SOP %s: %v", sop.Name, err)
		}
		if err := json.Unmarshal(metaJSON, &sop.Metadata); err != nil {
			log.Printf("automation: unmarshal metadata for SOP %s: %v", sop.Name, err)
		}
		if sop.Metadata == nil {
