...
func (s *Store) validateID(id string) error {
	if strings.Contains(id, "/") || strings.Contains(id, "\\") || strings.Contains(id, "..") {
		return fmt.Errorf("tasks: invalid id %q", id)
	}
	return nil
}

// Delete removes a task file by ID. No-op when the task doesn't
// exist (idempotent — delete what isn't there is still success).
