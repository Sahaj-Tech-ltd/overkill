...
type RenderConfig struct {
	...
	Name string
	// DiagramMermaid is an optional Mermaid diagram string to embed in the output.
...
func RenderPlan(markdown []byte, cfg RenderConfig) string {
	...
	return buildHTML(title, toc, htmlBody, fileTree, time.Now().UTC(), cfg.DiagramMermaid)
}
...
func sanitizeForHTML(s string) string {
	// Only escape sequences that would break the HTML embedding,
	// leaving Mermaid syntax intact.
	s = strings.ReplaceAll(s, "</div>", "<\\/div>")
...
func buildHTML(title string, toc []tocItem, body string, fileTree []fileTreeEntry, genTime time.Time, diagramMermaid string) string {
...
<div class="mermaid">
`)
		b.WriteString(sanitizeForHTML(diagramMermaid))
		b.WriteString(`
</div>
<p class="diagram-caption">Architecture Overview</p>
</div>
