#!/bin/sh
# install.sh — POSIX installer for ethos.
#
# Detects platform, prefers `go install` when Go is on PATH, otherwise
# downloads a pre-built binary, drops it in $HOME/.local/bin/ethos (or
# $ETHOS_INSTALL_DIR), and bootstraps ~/.ethos/.
#
# Env knobs:
#   ETHOS_INSTALL_DIR=/path        override install location
#   ETHOS_BUILD_FROM_SOURCE=1      force `go install` path
...
REPO="Sahaj-Tech-ltd/ethos"
GO_PKG="github.com/Sahaj-Tech-ltd/ethos/cmd/ethos"
RELEASE_BASE="https://github.com/${REPO}/releases/latest/download"

INSTALL_DIR="${ETHOS_INSTALL_DIR:-${HOME}/.local/bin}"
ETHOS_HOME="${HOME}/.ethos"
...
step_build_from_source() {
    BIN_PATH="${INSTALL_DIR}/ethos"
    if [ -n "${ETHOS_LOCAL_SOURCE:-}" ] && [ -d "${ETHOS_LOCAL_SOURCE}" ]; then
        say "building from local source at ${ETHOS_LOCAL_SOURCE}"
        ( cd "${ETHOS_LOCAL_SOURCE}" && go build -o "$BIN_PATH" ./cmd/ethos ) \
        ...
        GOBIN="$INSTALL_DIR" go install "${GO_PKG}@latest" \
            || die "go install failed (repo may not yet be published; try ETHOS_LOCAL_SOURCE=/path/to/checkout)"
    fi
...
step_download_binary() {
    need curl
    asset="ethos-${PLATFORM}-${ARCH}"
    url="${RELEASE_BASE}/${asset}"
    sha_url="${url}.sha256"
    tmp=$(mktemp -d 2>/dev/null || mktemp -d -t ethos)
    trap 'rm -rf "$tmp"' EXIT

    say "downloading ${url}"
    if ! curl -fsSL "$url" -o "${tmp}/ethos"; then
        die "failed to download $url — try ETHOS_BUILD_FROM_SOURCE=1 with Go on PATH"
    fi

    # SHA256 verify if a sibling .sha256 exists upstream.
    if curl -fsSL "$sha_url" -o "${tmp}/ethos.sha256" 2>/dev/null; then
        say "verifying SHA256"
        expected=$(awk '{print $1}' "${tmp}/ethos.sha256")
        if command -v sha256sum >/dev/null 2>&1; then
            actual=$(sha256sum "${tmp}/ethos" | awk '{print $1}')
        elif command -v shasum >/dev/null 2>&1; then
            actual=$(shasum -a 256 "${tmp}/ethos" | awk '{print $1}')
        else
    ...
    BIN_PATH="${INSTALL_DIR}/ethos"
    if [ "${SUDO:-}" = "sudo" ]; then
        sudo mv "${tmp}/ethos" "$BIN_PATH"
    else
        mv "${tmp}/ethos" "$BIN_PATH"
    fi
    say "installed binary to $BIN_PATH"
}

step_install() {
    if [ "${ETHOS_BUILD_FROM_SOURCE:-0}" = "1" ]; then
        command -v go >/dev/null 2>&1 || die "ETHOS_BUILD_FROM_SOURCE=1 set but 'go' not on PATH"
        step_build_from_source
        return
    fi
...
step_bootstrap_home() {
    ensure_dir "$ETHOS_HOME"
    for sub in sessions plugins cache journal/raw memories; do
        ensure_dir "${ETHOS_HOME}/${sub}"
    done

    cfg="${ETHOS_HOME}/config.toml"
    if [ ! -f "$cfg" ]; then
        cat > "$cfg" <<'EOF'
# ethos config — edit freely. Run `ethos doctor` to validate.
...
step_finish() {
    ...
    printf '\nNext steps:\n'
    printf '  1. run `ethos` to launch the TUI\n'
    printf '  2. run `ethos doctor` to self-check every subsystem\n'
    printf '  3. run `ethos --help` for the CLI surface\n'
}
