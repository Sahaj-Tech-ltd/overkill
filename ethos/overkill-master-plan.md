...
| **Phase 5** Advanced R&D (§8.1-8.6) | ✅ closed 2026-05-14 | §8.2/§8.3/§8.4/§8.5 ✅ fully; §8.1 + §8.6 application-layer approximations (KV-cache + gradient-based originals explicitly out of scope) |
...
- [x] Drift detection: flag when agent behavior diverges from norms — `internal/drift/drift.go` tracks per-metric running stats via Welford's online algorithm (O(1) memory, numerically stable). Tracked axes: tool_calls_per_turn, error_rate, recovery_rate, session_length, avg_turn_duration_seconds. `Baseline.Compare` flags metrics whose |z-score| exceeds threshold (default 2.0) after MinSessions warmup (default 5). Findings sort by absolute drift descending. `Store` persists baseline atomically.
...
- [x] LATS-style tree search for multi-path code exploration — Zhou 2024 — `internal/lats/lats.go` ships a lightweight N-branch racer. Caller declares candidate Branches; Runner drives each (typically a subagent in its own worktree); Scorer ranks results. `Race()` returns BranchResults highest-score-first. `DefaultScorer` rewards completion + non-empty response, penalizes errors/timeouts. `CancelLosersOnWin` opt-in lets the winner abort still-running losers. WorktreeProvider hook composes with the §8.5 worktree manager so branches don't trample each other's filesystem. **Honest scope:** the paper runs full MCTS with rollouts + UCB; we ship N-way racing first. Full MCTS remains an open target.

### 8.6 RL-based Self-Improvement  ⚠️ Reflexion landed; credit assignment shipped as application-layer approximation
- [x] Credit assignment across long coding trajectories — Zhang 2026 — **application-layer approximation** (not gradient-based — the paper requires training infrastructure). `internal/credit/credit.go` ships `Analyzer` that folds SessionRecord (outcome + actions) and produces per-action stats: SuccessCount / FailureCount / Lift / Confidence. `Lift = P(success | action) / P(success | no action)`; > 1.0 correlates with success, < 1.0 with failure. Confidence buckets by support count (low <5, medium 5–19, high 20+). `SuccessCorrelated` / `FailureCorrelated` filter by lift threshold + confidence floor. Per-session-per-action dedup so a session calling Read 10 times only bumps once. Atomic JSONL store persists session records across daemon restarts. **Honest scope:** correlation not causation; useful for surfacing patterns the operator wouldn't spot manually but not a learned policy.
...
Distilled from the Mistral Vibe + Medium 3.5 announcement (2026-05) — what's worth stealing, and what's load-bearing for our own bridges. Companion wish: `wishes.md` #7 (session teleport) is the missing primitive that ties this section together.

### 8.7.1 Suspend / Notify / Resume — approval-while-away
...
### 8.7.6 User Is Source of Truth  _(from wishes.md #6)_

When the user states something as fact the model can't verify from training — **search first, correct never**.
