...
edit `cmd/ethos/root.go` for each release. The `Version` `var` defaults
...
# 2. Tag and push.
git tag -a vX.Y.Z -m "ethos vX.Y.Z"
...
1. Cross-compiles `ethos` for `linux/amd64`, `linux/arm64`,
...
curl -fsSL https://raw.githubusercontent.com/Sahaj-Tech-ltd/ethos/vX.Y.Z/install.sh | sh
ethos --version    # → "ethos vX.Y.Z"
ethos --help
```
...
```bash
curl -fsSL -O https://github.com/Sahaj-Tech-ltd/ethos/releases/download/vX.Y.Z/SHA256SUMS
curl -fsSL -O https://github.com/Sahaj-Tech-ltd/ethos/releases/download/vX.Y.Z/ethos-vX.Y.Z-linux-amd64
...
git tag -a vX.Y.1 -m "ethos vX.Y.1"
