// Package main — `ethos snapshot` manages BadgerDB session snapshots
// (master plan §4.20). Snapshots live under ~/.ethos/snapshots; the daemon
...
//   - export:  write a human-readable memory dump to ~/.ethos/exports/
package main
...
var snapshotListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(names) == 0 {
			fmt.Printf("%sno snapshots yet — run 'ethos snapshot create'%s\n", colorYellow, colorReset)
			return nil
		}
		for _, n := range names {
			fmt.Println(n)
...
var snapshotRestoreCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		defer cleanup()
		path := args[0]
		// Accept either a bare filename (look up under ~/.ethos/snapshots) or
		// a fully-qualified path.
		if !filepath.IsAbs(path) {
			home, _ := os.UserHomeDir()
			path = filepath.Join(home, ".ethos", "snapshots", path)
		}
...
var snapshotExportCmd = &cobra.Command{
	Use:   "export",
	Short: "Write a human-readable memory export under ~/.ethos/exports/",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		defer cleanup()
		home, _ := os.UserHomeDir()
		exportDir := filepath.Join(home, ".ethos", "exports")
...
func openSnapshotManager() (*session.SnapshotManager, func(), error) {
	...
	home, _ := os.UserHomeDir()
	dir := filepath.Join(home, ".ethos", "snapshots")
	return session.NewSnapshotManager(store, dir, defaultMaxSnapshots), cleanup, nil
}

// openSessionStore opens the per-user session store at ~/.ethos/sessions.
...
func openSessionStore() (*session.BadgerStore, func(), error) {
	...
	dir := filepath.Join(home, ".ethos", "sessions")
...
func writeMemoryExport(snapshotPath, outPath string) error {
	...
	hdr := fmt.Sprintf("# Ethos Memory Export\n\n- Snapshot: %s\n- Generated: %s\n- Bytes: %d\n\n",
	...
	body := "_This file is a human-friendly companion to the binary snapshot. " +
		"To restore, run `ethos snapshot restore " + filepath.Base(snapshotPath) + "`._\n"
...
func SnapshotDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".ethos", "snapshots")
}
