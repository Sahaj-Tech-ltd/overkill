...
func buildTUIApp() *tui.App {
	...
	// Auto-inject CODEBASE.md from ~/.ethos/introspection/ into the system
	// prompt when present (master plan §4.18). Generated by /init.
	if home, herr := os.UserHomeDir(); herr == nil {
		introDir := filepath.Join(home, ".ethos", "introspection")
		a.SetContextProvider(func(ctx context.Context, sid string) string {
			return introspection.LoadCodebaseSnippet(introDir, 8000)
		})
	}
