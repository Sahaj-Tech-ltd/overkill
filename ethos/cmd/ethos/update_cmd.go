// Package main — `ethos update` (master plan §7.6).
//
...
//
// Repo coordinates are configured via env (ETHOS_UPDATE_REPO=owner/name);
// fallback is "Sahaj-Tech-ltd/ethos". Set ETHOS_UPDATE_REPO="" to disable.
package main
...
const defaultUpdateRepo = "Sahaj-Tech-ltd/ethos"
...
var updateCmd = &cobra.Command{
	Use:   "update",
	Short: "Check for and install a newer ethos release (master plan §7.6)",
	RunE:  runUpdate,
}

func init() {
	updateCmd.Flags().BoolVar(&updateCheckOnly, "check", false, "report availability without installing")
	updateCmd.Flags().StringVar(&updateRepo, "repo", "", "GitHub repo (owner/name); falls back to ETHOS_UPDATE_REPO or built-in default")
...
func CheckUpdateAsync() {
	...
	go func() {
		...
		if isNewer(rel.TagName, Version) {
			fmt.Fprintf(os.Stderr, "%s[update] new release available: %s → %s — run `ethos update`%s\n",
				colorBlue, Version, rel.TagName, colorReset)
		}
	}()
}
...
func resolveUpdateRepo() string {
	...
	if v, ok := os.LookupEnv("ETHOS_UPDATE_REPO"); ok {
		return v
	}
