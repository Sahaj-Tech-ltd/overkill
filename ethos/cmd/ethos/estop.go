// Package main — `ethos estop` (master plan §7.1) is the emergency stop.
//
// Sends SIGTERM to:
//   - the running daemon (via ~/.ethos/daemon.pid)
//   - any other ethos processes the user is running, identified by reading
//     ~/.ethos/sessions/*/owner.pid sentinel files (best-effort)
//
...
var estopCmd = &cobra.Command{
	Use:   "estop",
	Short: "Emergency-stop every running ethos process",
	Long: `Sends SIGTERM to the daemon and any session owners listed under
~/.ethos/sessions/*/owner.pid. Escalates to SIGKILL after 3 seconds for
...
func collectEstopTargets() []int {
	...
	if home, err := os.UserHomeDir(); err == nil {
		root := filepath.Join(home, ".ethos", "sessions")
