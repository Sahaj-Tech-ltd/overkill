...
func buildTUIApp() *tui.App {
	...
	// §6.5 Wall 2: generate OVERKILL_ARCH.md + CONTEXT.md in the
	// project root on first boot in this directory. The wall reads
	// these files; without them the wall scores against nothing.
	// Both functions are no-ops when files already exist.
	if cwd, werr := os.Getwd(); werr == nil {
		if _, err := walls.EnsureArch(cwd); err != nil {
			log.Printf("walls: ensure arch: %v", err)
		}
		if _, err := walls.EnsureGlossary(cwd); err != nil {
			log.Printf("walls: ensure glossary: %v", err)
		}
	}

	// Tools that surface the §6.5 files to the agent. Project root
	// resolved lazily so a `cd` mid-session is respected.
	...
	toolReg.Register(tools.NewArchReadTool(projectRoot))
	toolReg.Register(tools.NewGlossaryReadTool(projectRoot))
	toolReg.Register(tools.NewGlossaryAddTermTool(projectRoot))
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		...
		a.SetContextProvider(func(ctx context.Context, sid string) string {
			parts := []string{
				...
				loadArchSnippet(4000),
				loadGlossarySnippet(2000),
			}
			if orders != nil {
	...
	// Auto mode (master plan §7.3): autonomous plan-driven execution.
	// Loads autonomy level from config; "safe"/"yolo"/"auto" enable the
	// auto-mode controller. "supervised"/"full"/"readonly" use legacy
	// behavior. Use the /auto command to toggle at runtime.
	if cfg != nil && cfg.Security.AutonomyLevel != "" {
