...
var configSetCmd = &cobra.Command{
	...
	Example: "  overkill config set agent.name Butter\n" +
		"  overkill config set security.autonomy_level full\n" +
		"  overkill config set cost.daily_limit_usd 10.0",
	RunE: func(cmd *cobra.Command, args []string) error {
		key := args[0]
		value := args[1]
...
var configGetCmd = &cobra.Command{
	...
	Example: "  overkill config get agent.name\n" +
		"  overkill config get security.autonomy_level",
	RunE: func(cmd *cobra.Command, args []string) error {
		key := args[0]
		parts := strings.Split(key, ".")
		if len(parts) < 2 {
