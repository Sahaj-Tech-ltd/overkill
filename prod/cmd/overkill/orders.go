// Package main — `overkill orders add|rm|list|toggle` manages standing orders
// (master plan §7.1). Stored at ~/.overkill/standing-orders.jsonl; loaded by
...
var ordersListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		all := o.All()
		if len(all) == 0 {
			fmt.Printf("%sno standing orders yet — `overkill orders add \"...\"`%s\n", colorYellow, colorReset)
			return nil
		}
		for _, so := range all {
			marker := " "
...
func openOrders() (*automation.OrdersFile, error) {
	...
	return automation.NewOrdersFile(filepath.Join(home, ".overkill", "standing-orders.jsonl"))
}
