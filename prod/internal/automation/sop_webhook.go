...
type SOPWebhookServer struct {
	...
	server    *http.Server
	serverCtx context.Context // server lifecycle context; derived for SOP goroutines
}
...
func (s *SOPWebhookServer) Run(ctx context.Context) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("automation: sop webhook ListenAndServe panic: %v\n%s", r, debug.Stack())
			}
		}()
...
func (s *SOPWebhookServer) handleTrigger(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		go func() {
			defer func() {
				if r := recover(); r != nil {
					log.Printf("automation: sop executor goroutine panic: %v\n%s", r, debug.Stack())
				}
			}()
			ctx, cancel := context.WithTimeout(s.serverCtx, 30*time.Minute)
			defer cancel()
