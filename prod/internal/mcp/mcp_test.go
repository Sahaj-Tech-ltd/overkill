...
// TestJSONRPCFailAllPendingNil verifies that failAllPending does not panic
// when called with a nil error (simulating MCP server clean exit).
// Regression test for #96: scanner.Err() returns nil on clean exit,
// and failAllPending(nil) would call nil.Error() → panic.
func TestJSONRPCFailAllPendingNil(t *testing.T) {
	cR, cW := io.Pipe()
	conn := newJSONRPCConn(cW, cR)
	...
	for i := int64(1); i <= 3; i++ {
		ch := make(chan *jsonrpcMessage, 1)
		conn.pending.Store(i, ch)
	}

	// This should NOT panic — server exited cleanly, no error.
	// drain the channels to avoid goroutine leak
	go func() {
		conn.pending.Range(func(k, v any) bool {
			<-v.(chan *jsonrpcMessage)
			return true
		})
	}()
	conn.failAllPending(nil)
}

// TestJSONRPCReadLoopCleanExit verifies that readLoop handles a clean
// stdin close (server exit) without panicking in failAllPending.
func TestJSONRPCReadLoopCleanExit(t *testing.T) {
	cR, cW := io.Pipe()
	conn := newJSONRPCConn(cW, cR)
	_ = cW.Close() // close write end to simulate server exit

	err := conn.readLoop()
	// On clean exit, scanner.Err() returns nil, so readLoop returns nil.
	// The important thing is it doesn't panic.
	if err != nil {
		t.Logf("readLoop returned: %v (expected nil on clean close)", err)
	}
	if !conn.closed.Load() {
		t.Error("expected closed flag to be set after readLoop exits")
	}
}

// TestJSONRPCTypedNilError verifies that Call does not return a non-nil
// error interface wrapping a nil concrete *jsonrpcError (Go typed-nil footgun).
func TestJSONRPCTypedNilError(t *testing.T) {
	// Use a single pipe: conn reads from pr, we write the response to pw.
	pr, pw := io.Pipe()
	conn := newJSONRPCConn(io.Discard, pr) // write side isn't used in this test
	go func() { _ = conn.readLoop() }()
	defer conn.close()

	// Send a valid response with null error
	msg := jsonrpcMessage{
		JSONRPC: "2.0",
		ID:      json.RawMessage("1"),
		Result:  json.RawMessage(`{"ok":true}`),
		Error:   nil, // explicitly nil
	}
	...
	defer cancel()
	result, err := conn.Call(ctx, "test", nil)
	if err != nil {
		t.Fatalf("unexpected error from Call (typed-nil?): %v", err)
	}
