...
const (
	...
	defaultTransferSh = "https://transfer.sh"
)

// transferShEndpoint returns the transfer.sh-compatible upload endpoint.
// It honours the TRANSFER_ENDPOINT env var so users can point at a
// self-hosted instance (transfer.sh is deprecated).  Falls back to the
// public default when the env var is unset or empty.
func transferShEndpoint() string {
	if ep := os.Getenv("TRANSFER_ENDPOINT"); ep != "" {
		return ep
	}
	return defaultTransferSh
}
