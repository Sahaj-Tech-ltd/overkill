...
func (c *Client) Start(ctx context.Context) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("[plugin] conn.Serve goroutine panic: %v\n%s", r, debug.Stack())
			}
		}()
		if err := c.conn.Serve(ctx); err != nil {
			log.Printf("[plugin] serve error: %v", err)
		}
	}()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("[plugin] stderr drain goroutine panic: %v\n%s", r, debug.Stack())
			}
		}()
		buf := make([]byte, 4096)
		for {
