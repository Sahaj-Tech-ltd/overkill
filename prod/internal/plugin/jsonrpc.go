...
func (c *Conn) dispatch(ctx context.Context, msg *rpcMessage) {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("plugin: JSON-RPC handler goroutine panic: %v\n%s", r, debug.Stack())
			}
		}()
...
func (c *Conn) writeMessage(msg *rpcMessage) error {
	...
	b = append(b, '\n')
	// Marshal and prepare the payload before acquiring the write lock,
	// so the lock is only held for the actual write, not during JSON
	// encoding or any other potentially slow operation.
	c.writeMu.Lock()
	_, err = c.w.Write(b)
	c.writeMu.Unlock()
	if err != nil {
