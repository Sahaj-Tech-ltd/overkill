...
// Starts a background goroutine that prunes memories older than 90 days
// every hour. Override with SetRetention().
func NewPostgresStore(db *sql.DB) (*PostgresStore, error) {
	s := &PostgresStore{db: db, retention: 90 * 24 * time.Hour} // B001: default 90-day retention
...
func (s *PostgresStore) Retrieve(ctx context.Context, query Query) (*SearchResult, error) {
	...
	// M2: Increased overscan from 10x to 50x to cover more candidates.
	// For limit=10, this gives 500 candidates instead of 100.
	overscan := limit * 50
	if overscan > 1000 {
		overscan = 1000
	}
	...
	sort.Slice(memories, func(i, j int) bool {
		if memories[i].Relevance != memories[j].Relevance {
			return memories[i].Relevance > memories[j].Relevance
		}
		// M1: When relevance is equal (e.g., empty query), fall back to timestamp DESC.
...
func (s *PostgresStore) List(ctx context.Context, opts ListOptions) ([]Memory, error) {
	...
	if opts.Limit <= 0 {
		opts.Limit = 100 // default safety cap
	}
