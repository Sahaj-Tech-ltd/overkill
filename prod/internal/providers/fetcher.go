...
var providerEndpoints = map[string]providerEndpoint{
	...
	"togetherai": {modelsPath: "/api/v1/models", authStyle: "bearer"},
...
func (f *HTTPModelFetcher) fallback(providerType string) []Model {
	...
	case "custom":
		return nil // custom providers define their own models in config
	default:
		return nil // unknown provider type — caller should use configured models
	}
}
