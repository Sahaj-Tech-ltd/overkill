...
type Bot struct {
	...
	SeenTTL    time.Duration // dedup TTL; default 10min (from WhatsAppCloudConfig.SeenTTLSec)
...
func (b *Bot) alreadySeen(id string) bool {
	...
	if ts, ok := b.seenIDs[id]; ok && now.Sub(ts) < ttl {
		return true
	}
...
func (b *Bot) handleMessage(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				b.Logger.Printf("whatsapp-cloud: panic in webhook: %v", r)
			}
		}()
		b.processPayload(payload)
	}()
}
...
const whatsappMaxLen = 4096 // WhatsApp Cloud API text body limit
...
func (r *cloudReply) Final(ctx context.Context, _ string, text string) error {
	for len(text) > 0 {
		var chunk string
		chunk, text = gateway.ChunkAtRune(text, whatsappMaxLen)
		if err := r.bot.sendMessage(ctx, r.to, chunk); err != nil {
			return err
		}
	}
	return nil
