...
type Bot struct {
	...
	// AllowedRooms restricts inbound messages to specific room IDs.
	// Empty map (default) means all rooms are allowed.
	...
	// Configurable TTL overrides (from MatrixConfig).
	SeenTTL        time.Duration // dedup TTL; default 10min
	MemberCountTTL time.Duration // member count cache TTL; default 5min
}
...
func (b *Bot) alreadySeen(eventID string) bool {
	...
	if ts, ok := b.seenEvents[eventID]; ok && time.Since(ts) < ttl {
		return true
	}
...
func (b *Bot) Run(ctx context.Context) error {
	...
	// Periodically evict expired member count cache entries.
	go b.memberCountCleaner(ctx)
	...
	for {
		...
		if err != nil {
			...
			// Check for permanent auth errors — don't retry 401/403.
			if strings.Contains(err.Error(), "401") || strings.Contains(err.Error(), "403") {
				b.authMu.Lock()
				b.authErr = err
				b.authMu.Unlock()
				b.Logger.Printf("matrix: permanent auth error, exiting sync loop: %v", err)
				return err
			}
...
func (b *Bot) processEvents(syncResp *matrixSyncResponse) {
	...
	for roomID, room := range syncResp.Rooms.Join {
		for _, event := range room.Timeline.Events {
			...
			// Allowlist check: if AllowedRooms is configured, only accept
			// messages from listed rooms.
			if len(b.AllowedRooms) > 0 && !b.AllowedRooms[roomID] {
				b.Logger.Printf("matrix: skip disallowed room %s", roomID)
				continue
			}
			...
			b.runCtxMu.Unlock()
			if dispatchCtx == nil {
				b.Logger.Printf("matrix: rejecting message — bot.Run() not yet started (runCtx is nil)")
				return
			}
...
// memberCountCleaner periodically evicts expired member count cache entries
// to prevent unbounded memory growth on long-running bots.
func (b *Bot) memberCountCleaner(ctx context.Context) {
	ttl := b.MemberCountTTL
	if ttl == 0 {
		ttl = memberCountTTL
	}
	...
	for {
		...
		case <-ticker.C:
			b.memberMu.Lock()
			now := time.Now()
			for roomID, tc := range b.memberCounts {
				if now.Sub(tc.ts) > ttl {
					delete(b.memberCounts, roomID)
				}
			}
			b.memberMu.Unlock()
		}
...
func (b *Bot) fetchMemberCount(roomID string) int {
	// Derive the HTTP context from the bot's Run context so the
	// request is cancelled when the bot shuts down.
	b.runCtxMu.Lock()
	parentCtx := b.runCtx
	b.runCtxMu.Unlock()
	if parentCtx == nil {
		b.Logger.Printf("matrix: fetchMemberCount — bot.Run() not yet started, returning 0")
		return 0
	}
...
const matrixMaxLen = 60000 // conservative limit below the 65536 spec max
...
// We truncate the text for safety so an Update call never silently
// drops bytes without the caller knowing.
...
// indicator followed by the final response. Long responses are split
// into multiple messages to stay within the Matrix event size limit.
func (r *matrixReply) Final(ctx context.Context, handle, text string) error {
	...
	for len(text) > 0 {
		var chunk string
		chunk, text = gateway.ChunkAtRune(text, matrixMaxLen)
		if _, err := r.bot.sendMessage(ctx, r.roomID, chunk); err != nil {
			return err
		}
	}
	return nil
