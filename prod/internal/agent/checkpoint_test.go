...
func TestCheckpoint_RollbackConcurrent(t *testing.T) {
	// Bug #7: Rollback() did not hold the CheckpointManager mutex,
	// causing concurrent git operations to race. This test spawns
	// parallel goroutines calling Rollback to trigger the race detector.
	dir, cleanup := initTestRepo(t)
	defer cleanup()

	cm := NewCheckpointManager(dir)

	// Create 5 snapshots so we have room to roll back.
	for i := 0; i < 5; i++ {
		writeFile(t, dir, "marker.txt", fmt.Sprintf("v%d", i))
		if _, err := cm.Snapshot(fmt.Sprintf("snap-%d", i)); err != nil {
			t.Fatalf("Snapshot %d: %v", i, err)
		}
	}

	// Concurrent rollbacks on the same manager expose the missing mutex.
	// Use only Rollback(0) (reset to latest) which is always valid regardless
	// of how many commits other goroutines have already rolled back.
	const n = 10
...
func TestCheckpoint_RollbackWrongVariable(t *testing.T) {
	// Bug #8: Rollback() returned 'out' (git reset output) instead of
	// 'hashOut' (git rev-parse output) in the error path. The regression
	// test verifies that a successful Rollback returns a clean summary
	// string (with "rolled back" and "now at"), not raw git reset output
	// like "HEAD is now at ..." which would indicate the wrong variable.
	dir, cleanup := initTestRepo(t)
	defer cleanup()

	cm := NewCheckpointManager(dir)

	// Create a snapshot so we can roll back.
	writeFile(t, dir, "f.txt", "v1")
	if _, err := cm.Snapshot("test"); err != nil {
		t.Fatalf("Snapshot: %v", err)
	}

	summary, err := cm.Rollback(0)
	if err != nil {
		t.Fatalf("Rollback: %v", err)
	}

	// The summary should be a clean message, not raw git output.
	if !strings.Contains(summary, "rolled back") {
		t.Errorf("summary missing 'rolled back': %q", summary)
	}
	if !strings.Contains(summary, "now at") {
		t.Errorf("summary missing 'now at': %q", summary)
	}
	if strings.Contains(summary, "HEAD is now at") {
		t.Errorf("summary contains raw git reset output (wrong variable used): %q", summary)
	}
}
