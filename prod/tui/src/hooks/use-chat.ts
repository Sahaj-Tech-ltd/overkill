  ...
  if (output) {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        if (typeof parsed.lines_removed === "number")
          removed = parsed.lines_removed;
...
export function useChat(backend: BackendClient): UseChatResult {
  ...
  // B147: Store retry timeout ID so we can clear it on unmount, preventing
  // setState calls on unmounted components.
  const retryTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // B017: Clean up thinking timer and retry timeout on unmount so they
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      // $shell pass-through: execute commands directly, bypass agent loop.
      // $ git status / $shell ls -la — runs in CWD, output added as system msg.
      if (/^\\$(\\s|shell\\s)/.test(text)) {
        const cmdText = text.replace(/^\\$\\s*(shell\\s+)?/, "").trim();
      ...
      // /doom — TUI-only easter egg. Shows controls then launches DOOM in browser.
      if (/^\/doom/.test(text)) {
        const controlsMsg: Message = {
          id: newMsgId(),
          role: "system",
          content: `🎮 **DOOM**\n\nControls: **WASD** move · **Arrows** turn · **Space** fire · **E** use\n**1-7** weapons · **Shift** run · **ESC** exit\n\nHit **Ctrl+Enter** to launch.`,
          startTime: Date.now(),
        };
        setMessages((prev) => [...prev, controlsMsg]);

        // Launch DOOM after a brief pause so the user sees the controls.
        setTimeout(async () => {
          const port = process.env["OVERKILL_API_PORT"] ?? "8687";
          const { exec } = await import("node:child_process");
          const url = `http://localhost:${port}/doom`;
          ...
          exec(cmd, (err: any) => {
            if (err) {
              const errMsg: Message = {
                id: newMsgId(),
                role: "system",
                content: `❌ Couldn't open browser: ${err.message}`,
                startTime: Date.now(),
              };
              setMessages((prev) => [...prev, errMsg]);
            }
        ...
        // Return the user message as a system note so it doesn't hit the agent.
        const doomMsg: Message = {
          id: newMsgId(),
          role: "user",
          content: text,
          startTime: Date.now(),
        };
        setMessages((prev) => [...prev, doomMsg]);
        return;
      }
          ...
          switch (event.type) {
            ...
            case "tool_call":
              ...
              if (event.name && FILE_TOOL_NAMES.has(event.name)) {
                ...
                if (path) {
                  const { added, removed } = estimateDelta(
                    event.output,
                    event.input,
                  );
                  setFileChanges((prev) => {
                    ...
                    return [
                      ...prev,
                      { path, added, removed, timestamp: Date.now() },
                    ];
                  });
                }
              }
              ...
              setMessages((prev) => {
                ...
                if (updated.length > 0) {
                  updated[updated.length - 1] = {
                    ...
                    reasoningDuration:
                      reasoningDurationRef.current || undefined,
  ...
  const cancelMessage = useCallback(() => {
    ...
    backend.cancel();
    backend
      .call("agent.abort", { session_id: sessionIdRef.current })
      .catch((err: unknown) => {
        log.error("agent.abort failed:", err);
      });
  }, [backend]);
  ...
  const retryLastMessage = useCallback(() => {
    ...
    setMessages((prev) => {
      ...
      retryTimeoutRef.current = setTimeout(() => {
        sendMessage(text).catch((e: unknown) => {
          log.error("pop_and_resend: sendMessage failed:", e);
        });
      }, 0);
    ...
    backend.undo(sessionIdRef.current).catch((err: unknown) => {
      log.error("undo (retry) failed:", err);
    });
  }, [backend, sendMessage]);
