...
function buildTree(sessions: SessionInfo[]): TreeNode[] {
  ...
  for (const s of sessions) {
    if (s.parent_id && byId.has(s.parent_id)) {
      const siblings = childrenMap.get(s.parent_id) ?? [];
      siblings.push(s);
      childrenMap.set(s.parent_id, siblings);
    } else {
      roots.push(s);
    }
  }
...
export function SessionPanel({
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      // Fork the current session via BackendClient.fork() which calls agent.fork.
      const forkName = `forked-${Date.now()}`;
      backend
        .fork(currentSessionId, forkName)
        .then((result) => {
          fetchSessions();
    ...
    <Box flexDirection="column" overflow="hidden">
          ...
          return (
            <Box key={s.id} paddingX={1}>
              ...
              <Box flexDirection="column" flexGrow={1}>
                ...
                <Text dimColor>{formatDate(s.updated_at)}</Text>
              </Box>
            </Box>
          );
        })}
      ...
      {!loading && !error && sessions.length > 0 && (
        <Box paddingX={1} marginTop={1} flexDirection="column">
          ...
          {currentSessionId && <Text dimColor>f fork current session</Text>}
        </Box>
      )}
    </Box>
  );
