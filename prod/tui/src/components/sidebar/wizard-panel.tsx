...
function StarRating({ rating }: { rating: number }): React.JSX.Element {
  ...
  return <Text color={theme.warning}>{stars || "—"}</Text>;
}
      ...
      {options.map((opt) => {
        ...
        return (
          <Box key={opt.id} flexDirection="column" paddingX={2}>
            ...
            <Box paddingLeft={4}>
              <Text dimColor>
                {opt.description.slice(0, 60)}
                {opt.description.length > 60 ? "..." : ""}
              </Text>
            </Box>
            {opt.tags && opt.tags.length > 0 && (
              <Box paddingLeft={4}>
                <Text color={theme.muted}>Tags: {opt.tags.join(", ")}</Text>
              </Box>
            )}
          </Box>
        );
...
export function WizardPanel({ backend }: WizardPanelProps): React.JSX.Element {
  const { theme } = useTheme();
  ...
  const [selectedReviewProvider, setSelectedReviewProvider] = useState("");
    ...
    backend
      ...
      .then((result) => {
        ...
        if (result.recommended) {
          ...
          setSelectedReviewProvider(result.recommended.review_provider);
        }
        setError(null);
      })
      .catch((err: unknown) => {
  ...
  const handleUseRecommended = useCallback(() => {
    ...
    backend
      .call<{ status: string; message: string }>("wizard.quick-setup", {
        ...
        review_provider: selectedReviewProvider || rec.review_provider,
        review_model: rec.review_model,
      })
      .then((result) => {
  ...
  // Gated behind isFocused so typing Enter in the chat input doesn't trigger it.
  useInput(
    (input, key) => {
  ...
  return (
    <Box flexDirection="column" overflow="hidden">
      ...
      {loading && (
        <Box paddingX={1}>
          <Text color={theme.warning}>Loading catalog...</Text>
        </Box>
      )}
      ...
      {catalog && (
        <Box flexDirection="column" marginTop={1}>
          ...
          <WizardSection
            title="Review & Audit"
            options={catalog.review}
            recommendedId={catalog.recommended?.review_provider}
            selected={selectedReviewProvider}
            loading={loading}
            onSelect={setSelectedReviewProvider}
          />
        </Box>
      )}
      ...
      {catalog && (
        <Box flexDirection="column" paddingX={1} marginTop={1}>
          ...
          {catalog.recommended && (
            <Box paddingX={1} marginTop={1} flexDirection="column">
              ...
              <Box paddingLeft={2} flexDirection="column">
                ...
                <Text dimColor>TTS: {catalog.recommended.tts || "—"}</Text>
                <Text dimColor>
                  Database: {catalog.recommended.database || "—"}
                </Text>
                <Text dimColor>
                  Review: {catalog.recommended.review_provider || "—"}
                  {catalog.recommended.review_model
                    ? ` (${catalog.recommended.review_model})`
                    : ""}
                </Text>
              </Box>
          ...
          <Box marginTop={1} paddingX={1}>
            <Box>
              <Text color={applying ? "yellow" : "green"} bold inverse>
          ...
          {applyResult && (
            <Box paddingX={1} marginTop={1}>
              <Text color={applyResult.startsWith("✓") ? "green" : "red"}>
                {applyResult}
              </Text>
            </Box>
          )}
          ...
          {/* Footer hint */}
          <Box paddingX={1} marginTop={1}>
            <Text dimColor>Press Enter on "Use Recommended" to apply</Text>
          </Box>
        </Box>
      )}
    </Box>
