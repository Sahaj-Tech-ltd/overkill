...
export function StatusBar({
  ...
  if (dirDisplay.startsWith(process.env.HOME ?? "/home/user")) {
    dirDisplay =
      "~" + dirDisplay.slice((process.env.HOME ?? "/home/user").length);
  }
      ...
      <Box gap={2}>
        ...
        {(permissionsPending ?? 0) > 0 && (
          <Text color={theme.warning}>
            △ {permissionsPending} Permission
            {permissionsPending !== 1 ? "s" : ""}
          </Text>
        )}
        ...
        {lspLabel && (
          <Text>
            <Text color={lspConnected ? theme.success : theme.muted}>
              {lspDot}
            </Text>
        ...
        {connected === "connected" && <Text color={theme.muted}>/status</Text>}
      </Box>
    </Box>
  );
}
