...
export function ConfigTab({
    ...
    (path: string[], label: string, currentVal: boolean) => {
      return (v: boolean) => {
        ...
        backend
          ...
          .catch((err: unknown) => {
            log.error("config.update (persistBool) failed:", err);
            // Rollback on failure
            setConfig((prev) => {
              if (!prev) return prev;
              const next = structuredClone(prev);
    ...
    (path: string[], label: string, currentVal: string) => {
      return (v: string) => {
        ...
        backend
          .call("config.update", { patch: buildPatch(path, v) })
          .catch((err: unknown) => {
            log.error("config.update (persistString) failed:", err);
            setConfig((prev) => {
              if (!prev) return prev;
              const next = structuredClone(prev);
              if (isRecord(next)) setNested(next, path, currentVal);
    ...
    const settings: Setting[] = [
      ...
      },
      {
        ...
        onChange: (v: string) => {
          ...
          isDirtyRef.current = true;
          changeSummaryRef.current.push(
            `Max output tokens: ${agent.max_output_tokens ?? "unlimited"} → ${v}`,
          );
          setConfig((prev) => {
            if (!prev) return prev;
            const next = structuredClone(prev);
            if (isRecord(next))
              setNested(next, ["agent", "max_output_tokens"], numVal);
            return next;
          });
          backend
            .call("config.update", {
              patch: buildPatch(["agent", "max_output_tokens"], numVal),
            })
            .catch((err: unknown) => {
              log.error("config.update (max_output_tokens) failed:", err);
              setConfig((prev) => {
                if (!prev) return prev;
                const next = structuredClone(prev);
                if (isRecord(next))
      ...
      },
      {
        ...
        onChange: (v: string) => {
          const numVal: number | null =
            v === "none" ? null : parseInt(v.replace(/^\$/, ""), 10);
          isDirtyRef.current = true;
          changeSummaryRef.current.push(
            `Daily cost limit: ${cost.daily_limit != null ? `$${cost.daily_limit}` : "none"} → ${v}`,
          );
          setConfig((prev) => {
            if (!prev) return prev;
            const next = structuredClone(prev);
            if (isRecord(next))
              setNested(next, ["cost", "daily_limit"], numVal);
            return next;
          });
          backend
            .call("config.update", {
              patch: buildPatch(["cost", "daily_limit"], numVal),
            })
            .catch((err: unknown) => {
              log.error("config.update (daily_limit) failed:", err);
              setConfig((prev) => {
                if (!prev) return prev;
                const next = structuredClone(prev);
                if (isRecord(next))
                  setNested(next, ["cost", "daily_limit"], cost.daily_limit);
                return next;
              });
            });
        },
      ...
      {
        id: "compaction.soft_trigger",
        label: "Compaction soft trigger",
        type: "managedEnum" as const,
        value:
          compaction.soft_trigger != null
            ? `${compaction.soft_trigger}%`
            : "off",
        onChange: (v: string) => {
          const numVal: number = v === "off" ? 0 : parseInt(v, 10);
          isDirtyRef.current = true;
          changeSummaryRef.current.push(
            `Compaction soft trigger: ${compaction.soft_trigger ?? 0}% → ${v}`,
          );
          setConfig((prev) => {
            if (!prev) return prev;
            const next = structuredClone(prev);
            if (isRecord(next))
              setNested(next, ["compaction", "soft_trigger"], numVal);
            return next;
          });
          backend
            .call("config.update", {
              patch: buildPatch(["compaction", "soft_trigger"], numVal),
            })
            .catch((err: unknown) => {
              log.error("config.update (compaction.soft_trigger) failed:", err);
              setConfig((prev) => {
                if (!prev) return prev;
                const next = structuredClone(prev);
                if (isRecord(next))
      ...
      {
        id: "compaction.hard_trigger",
        label: "Compaction hard trigger",
        type: "managedEnum" as const,
        value:
          compaction.hard_trigger != null
            ? `${compaction.hard_trigger}%`
            : "off",
        onChange: (v: string) => {
          const numVal: number = v === "off" ? 0 : parseInt(v, 10);
          isDirtyRef.current = true;
          changeSummaryRef.current.push(
            `Compaction hard trigger: ${compaction.hard_trigger ?? 0}% → ${v}`,
          );
          setConfig((prev) => {
            if (!prev) return prev;
            const next = structuredClone(prev);
            if (isRecord(next))
              setNested(next, ["compaction", "hard_trigger"], numVal);
            return next;
          });
          backend
            .call("config.update", {
              patch: buildPatch(["compaction", "hard_trigger"], numVal),
            })
            .catch((err: unknown) => {
              log.error("config.update (compaction.hard_trigger) failed:", err);
              setConfig((prev) => {
                if (!prev) return prev;
                const next = structuredClone(prev);
                if (isRecord(next))
  ...
  const filtered = useMemo(() => {
    ...
    return allSettings
      .map((s) => {
        ...
        const best = idMatch.score >= labelMatch.score ? idMatch : labelMatch;
    ...
    if (setting.id === "thinking.budget_tokens" && config) {
      return (
        <Box key={setting.id} paddingX={1}>
          ...
          <Text dimColor>: </Text>
          <Text>
            {config.thinking?.budget_tokens != null
              ? `${config.thinking.budget_tokens} tokens`
              : "not set"}
          </Text>
        </Box>
      );
  ...
  const footer = isSearchMode ? (
    <Box paddingX={1} marginTop={0}>
      <Text dimColor>type to filter · ↓ to list · Esc to close</Text>
    </Box>
  ) : (
    <Box paddingX={1} marginTop={0}>
      <Text dimColor>
        ↑↓ navigate · Space toggle · ←→ cycle · / search · Enter select · Esc
        close
      </Text>
    </Box>
  );
  ...
  if (loading) {
    return (
      <Box flexDirection="column" paddingX={1}>
        <Box marginBottom={1}>
          <Text color={theme.warning}>Loading configuration...</Text>
        </Box>
      </Box>
    );
  }
